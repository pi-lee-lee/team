...
def main():
    ...
    # 🔴 **로그 이름은 `offset` 이 아니라 *웹 포트* 로 정해진다.**
    #   `instance_tag()` 가 `".test+" + g_port_web` 다 — 오프셋 규약이 없어질 때(2026-08-20)
    #   **플래그는 고쳤는데 이 이름 공식이 안 따라왔다.**
    #   ⚠ 그래서 이 도구가 **옛 이름의 파일을 보고 "이번 기동분이 비어 있다"** 를 냈다 —
    #     🔴 **서버는 멀쩡히 떴는데 거짓 빨강**이다. 그것이 도구가 없는 것보다 나쁘다.
    #   🔑 한 결정(오프셋 폐지)이 **네 자리**에 흔적을 남겼다:
    #     `runtime.h` 안내문 · 배포 절차서 · `mock_node.py` 사용법 · **여기(이름 공식)**
    web_port = 9990 + a.offset
    log = "%s/parking-logs/parking-server.test+%d.log" % (home, web_port)
