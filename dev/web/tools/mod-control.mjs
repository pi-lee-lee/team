...
const URL_ = LIVE
  ...
  : ((fileArg
      ...
      : new URL('../../서머리/server/index.html', import.meta.url).href) + '?demo=1');
...
/* 🔴 **계약이 `measure` → `transition{value, r, age_ms}` 로 rename 된다**(SPEC-sensor-value §15.6).
   서버가 실제로 그것을 실어 보낸 뒤에 화면을 고친다 — 그때 **이 두 줄만** 바꾸면 이 파일이 따라온다.
   🔑 각본을 미리 짜 두면 **코드가 그것을 통과해야 하는 상태로 시작한다**(루트 지시).
   ⚠ 지금은 옛 이름이 정본이다. 이름을 미리 바꾸면 오늘 빨강이 되고, 그 빨강은 결함이 아니라 잡음이다. */
const TR_KEY = 'transition';            /* 계약 §15.3 — `{r, age_ms, value?}` */
const TR_NO_EVENT_TEXT = '변화 없음';    /* ① 키 없음 : 전이가 아직 없다 — **정상**이다 */
const TR_NO_VALUE_MARK = '못 쟀다';      /* 🔴 ② 전이는 났고 값이 없다 — ①과 **달라야** 한다 */
...
const MAP = {
  ...
  zones: [{ id: 'A1', kind: 'parking', cells: [[0, 0]], modules: [
    ...
    /* 🔑 **관측 모듈인데 measure 가 안 오는 경우**를 밟는다(SPEC-sensor-value §6) */
...
try {
  ...
  /* ══ 🔴 측정값 `measure` (SPEC-sensor-value §6) ═══════════════════════════════
     사용자 증상: *"초음파센서의 동작이 웹 화면에 표시되지 않는다."*
     원인은 둘이 겹쳐 있었다 — ① 값이 97.6% 안 왔다(장치) ② **화면이 그 키를 안 읽었다**(여기).
     🔑 ①이 ②의 증상을 삼켜서 *"안 뜨는 것이 정상"* 으로 보였다. 그래서 이 검사가 없었다. */
  {
    const M = BASE.map((m) => Object.assign({}, m));
    const byName = (n) => M.find((x) => x.name === n);
    byName('A1')[TR_KEY] = { r: true, value: 85, age_ms: 3000 };     /* ③ 전이 + 값 */
    byName('LC')[TR_KEY] = { r: false, value: 1234567, age_ms: 65000 }; /* 🔑 명령 모듈도 같은 칸 */
    byName('DR')[TR_KEY] = { r: true, value: 0, age_ms: 500 };       /* 🔴 **참인 0** — 접지 않는다 */
    /* B1(관측)·LD(명령)·ZZ(명령) 은 measure 없음 — 갈래 둘을 밟는다 */
    await evaluate(client, inject(ST(M)));
    await sleep(160);
    const READM = `(() => {
      const out = {};
      for (const li of document.querySelectorAll('#zone-detail .zmod')) {
        const t = li.textContent || '';
        const mm = t.match(/\(([A-Z0-9]{2})\)/);
        const key = mm ? mm[1] : t.slice(0, 8);
        const el = li.querySelector('.zmod__tr');
        out[key] = { meas: el ? el.textContent : null, unk: el ? (el.dataset.unknown === '1') : null };
      }
      return out;
    })()`;
    const mv = await evaluate(client, READM);
    console.log('  · measure → ' + JSON.stringify(mv));
    const drawn = Object.keys(mv || {}).filter((k) => mv[k].meas && !mv[k].unk);
    ok('🔴 분모: 측정값이 그려진 행이 하나 이상 (' + drawn.length + ')', drawn.length >= 1,
       JSON.stringify(mv) + ' — 0 이면 아래 대조가 공허하게 참이 된다');
    ok('🔴 전이 문장이 그려진다 (A1 → 찼다 · 85)',
       !!(mv.A1 && /찼다/.test(mv.A1.meas || '') && /85/.test(mv.A1.meas || '')), JSON.stringify(mv.A1));
    ok('🔑 `r` 이 **문장 앞**에 온다 (숫자가 앞이면 "지금 값" 으로 읽힌다)',
       !!(mv.A1 && /^(찼다|비었다)/.test((mv.A1.meas || '').trim())), JSON.stringify(mv.A1));
    ok('r:false 는 "비었다" 로 그린다 (LC)',
       !!(mv.LC && /^비었다/.test((mv.LC.meas || '').trim())), JSON.stringify(mv.LC));
    ok('나이를 서버 age_ms 로 그린다 (3000ms → "3초 전")',
       !!(mv.A1 && /3초 전/.test(mv.A1.meas || '')), JSON.stringify(mv.A1));
    ok('🔑 명령 모듈의 **상태값**도 그려진다 (LC → 1234567 · §14 state())',
       !!(mv.LC && /1234567/.test(mv.LC.meas || '')), JSON.stringify(mv.LC));
    ok('나이 분(分) 갈래 (65000ms → "1분 전")',
       !!(mv.LC && /1분 전/.test(mv.LC.meas || '')), JSON.stringify(mv.LC));
    ok('🔴 `measure.value === 0` 은 **참인 0** 이라 그려진다 (DR → 0)',
       !!(mv.DR && /(^|[^0-9])0([^0-9]|$)/.test(mv.DR.meas || '')), JSON.stringify(mv.DR));
    ok('🔴 관측 모듈 + 측정값 키 없음 → "' + TR_NO_EVENT_TEXT + '" (B1)',
       !!(mv.B1 && mv.B1.unk === true && mv.B1.meas === TR_NO_EVENT_TEXT), JSON.stringify(mv.B1));
    ok('🔴 명령 모듈 + measure 없음 → **칸 자체가 없다** (LD · 뜻 없는 문구를 안 붙인다)',
       !!(mv.LD && mv.LD.meas === null), JSON.stringify(mv.LD));

    /* 🔑 **음성 대조** — measure 를 다 빼면 값 칸이 사라지고 화면이 안 죽어야 한다. */
    await evaluate(client, inject(ST(BASE)));
    await sleep(160);
    const mv2 = await evaluate(client, READM);
    const rows2 = Object.keys(mv2 || {}).length;
    const drawn2 = Object.keys(mv2 || {}).filter((k) => mv2[k].meas && !mv2[k].unk);
    ok('🔑 음성 대조: measure 를 빼면 값 칸이 사라진다 (' + drawn2.length + '개)', drawn2.length === 0,
       JSON.stringify(mv2));
    ok('음성 대조: 그래도 모듈 행 수는 그대로다 (' + rows2 + ') — 화면이 안 죽는다',
       rows2 === Object.keys(mv || {}).length, JSON.stringify([rows2, Object.keys(mv || {}).length]));
    ok('음성 대조: 관측 모듈 둘은 "' + TR_NO_EVENT_TEXT + '" 으로 남는다 (A1 · B1)',
       !!(mv2.A1 && mv2.A1.unk === true && mv2.B1 && mv2.B1.unk === true), JSON.stringify([mv2.A1, mv2.B1]));
    /* 🔴 **분모** — 그 칸이 실제로 그려졌는가. 없으면 위 대조가 공허하게 참이 된다(원장 §5.88) */
    const unkRows = Object.keys(mv2 || {}).filter((k) => mv2[k].unk === true);
    ok('🔴 분모: "모름" 칸이 그려진 행이 하나 이상 (' + unkRows.length + ')', unkRows.length >= 1,
       JSON.stringify(mv2));

    /* ══ 🔴 음성 대조 ② — **나이가 매우 큰 값** (REQ-0344 · 루트 지시) ═══════════════
       개선 후 `age_ms` 는 **마지막 전이로부터**라 **분·시간 단위가 정상**이 된다.
       🔑 그래서 큰 나이가 **깨지지 않고 사람이 읽는 문구로** 나와야 한다.
       ⚠ 그리고 그것을 **이상으로 그리면 안 된다** — 문턱 경보를 만들지 않았음을 여기서 지킨다. */
    const M3 = BASE.map((m) => Object.assign({}, m));
    M3.find((x) => x.name === 'A1')[TR_KEY] = { r: true, value: 7, age_ms: 3600000 };  /* 1시간 */
    M3.find((x) => x.name === 'B1')[TR_KEY] = { r: false, value: 22, age_ms: 180000 }; /* 3분 */
    await evaluate(client, inject(ST(M3)));
    await sleep(160);
    const mv3 = await evaluate(client, READM);
    console.log('  · 큰 나이 → ' + JSON.stringify({ A1: mv3.A1, B1: mv3.B1 }));
    ok('🔴 분모: 큰 나이 각본에서 값이 그려진 행이 둘이다',
       !!(mv3.A1 && mv3.A1.meas && !mv3.A1.unk && mv3.B1 && mv3.B1.meas && !mv3.B1.unk),
       JSON.stringify([mv3.A1, mv3.B1]));
    ok('🔴 3,600,000ms → "1시간 전" (깨지지 않는다)',
       !!(mv3.A1 && /1시간 전/.test(mv3.A1.meas || '')), JSON.stringify(mv3.A1));
    ok('180,000ms → "3분 전"', !!(mv3.B1 && /3분 전/.test(mv3.B1.meas || '')), JSON.stringify(mv3.B1));
    ok('🔴 큰 나이를 **이상으로 표시하지 않는다** (unknown 표지가 안 붙는다 · 문턱 경보 0)',
       !!(mv3.A1 && mv3.A1.unk === false && mv3.B1 && mv3.B1.unk === false),
       JSON.stringify([mv3.A1, mv3.B1]) + ' — 전이가 없으면 나이가 커지는 것이 정상이다(§15.6)');

    /* ══ 🔴 음성 대조 ②-b — **전이는 났고 `value` 가 없다** (socket 경고 · REQ-0344) ═════════
       🔴 `if (transition.value != null)` 로 문을 열면 이 상태가 **① "변화 없음" 으로 접힌다** →
       *"찼는데 거리를 못 쟀다"* 와 *"아무 일도 없었다"* 가 **같은 모양**이 된다.
       🔑 그리고 그것이 **흔하다** — 오늘 A1 도착률 27.1%: **못 쟀는데 전이는 나는 것이 정상**이다. */
    const M5 = BASE.map((m) => Object.assign({}, m));
    M5.find((x) => x.name === 'A1')[TR_KEY] = { r: true, age_ms: 180000 };   /* value 키 **없음** */
    M5.find((x) => x.name === 'DR')[TR_KEY] = { r: false, age_ms: 4000 };    /* 명령 모듈도 같다 */
    /* B1 은 transition 자체가 없다 → ① 과 나란히 놓고 **다른지** 본다 */
    await evaluate(client, inject(ST(M5)));
    await sleep(160);
    const mv5 = await evaluate(client, READM);
    console.log('  · 전이 O · 값 X → ' + JSON.stringify({ A1: mv5.A1, DR: mv5.DR, B1: mv5.B1 }));
    ok('🔴 분모: 전이 문장이 그려진 행이 하나 이상',
       !!(mv5.A1 && mv5.A1.meas), JSON.stringify(mv5.A1));
    ok('🔴 전이 O · 값 X 를 **① 로 접지 않는다** (A1 ≠ "' + TR_NO_EVENT_TEXT + '")',
       !!(mv5.A1 && mv5.A1.meas !== TR_NO_EVENT_TEXT), JSON.stringify(mv5.A1));
    ok('🔴 그 자리에 **사건과 "' + TR_NO_VALUE_MARK + '" 가 같이 있다** (A1)',
       !!(mv5.A1 && /^찼다/.test((mv5.A1.meas || '').trim()) && mv5.A1.meas.includes(TR_NO_VALUE_MARK)),
       JSON.stringify(mv5.A1));
    ok('🔴 그리고 ①(전이 없음)과 **문구가 다르다** (A1 vs B1)',
       !!(mv5.A1 && mv5.B1 && mv5.A1.meas !== mv5.B1.meas && mv5.B1.meas === TR_NO_EVENT_TEXT),
       JSON.stringify([mv5.A1, mv5.B1]));
    ok('경과도 같이 그린다 (180,000 → "3분 전")',
       !!(mv5.A1 && /3분 전/.test(mv5.A1.meas || '')), JSON.stringify(mv5.A1));
    /* 🔴 **명령 모듈에는 "못 쟀다" 를 쓰지 않는다** — 값이 없는 것은 측정 실패가 아니라
       **상태 훅이 없다는 뜻**이다(재는 물건이 아니다). 원인을 지어내지 않는다.
       🔑 판별자는 `kind` 첫 글자 — ①에서 쓰는 것과 같은 것이다. */
    ok('🔴 명령 모듈은 사건과 경과만 그린다 (DR → 비었다 · 4초 전 · "' + TR_NO_VALUE_MARK + '" 없음)',
       !!(mv5.DR && /^비었다/.test((mv5.DR.meas || '').trim())
          && !mv5.DR.meas.includes(TR_NO_VALUE_MARK) && /4초 전/.test(mv5.DR.meas)),
       JSON.stringify(mv5.DR) + ' — 액추에이터에 "못 쟀다" 를 붙이면 원인을 지어내는 것이다');
    ok('🔴 그래도 관측 모듈에는 붙는다 (A1) — 두 갈래가 갈린다',
       !!(mv5.A1 && mv5.A1.meas.includes(TR_NO_VALUE_MARK)), JSON.stringify(mv5.A1));
    /* 🔴 `r` 이 계약을 어기면 조용히 정상으로 안 떨어뜨린다 */
    const M6 = BASE.map((m) => Object.assign({}, m));
    M6.find((x) => x.name === 'A1')[TR_KEY] = { r: 'yes', value: 5, age_ms: 1000 };
    await evaluate(client, inject(ST(M6)));
    await sleep(150);
    const mv6 = await evaluate(client, READM);
    ok('🔴 `r` 이 true/false 가 아니면 원문을 보인다 (서버 결함 신호)',
       !!(mv6.A1 && /알 수 없는 전이/.test(mv6.A1.meas || '') && /yes/.test(mv6.A1.meas || '')),
       JSON.stringify(mv6.A1));

    /* ══ 🔴 계약 상태 둘을 더 밟는다 (socket 실측 통보 · §15.6) ═══════════════════
       ㉠ **사건 낱말은 `transition.r` 에서만** 읽는다 — `value`(occ 비트)에서 읽으면
          둘이 갈릴 때 화면이 거짓말을 한다. **비트와 r 을 일부러 반대로** 넣어 가른다.
       ㉡ **`known:false` 인데 `transition` 이 있는 것은 정상**이다(노드가 끊겨도 그 사건은 일어났다).
          🔴 그때 표지가 없으면 **옛 사건을 현재로 읽는다** → `(마지막)` 이 붙어야 한다. */
    const M7 = BASE.map((m) => Object.assign({}, m));
    const a7 = M7.find((x) => x.name === 'A1');
    a7.value = true; a7.known = true;                                  /* 비트는 "찼다" */
    a7[TR_KEY] = { r: false, value: 3, age_ms: 2000 };                 /* 🔴 사건은 "비었다" */
    const l7 = M7.find((x) => x.name === 'LD');
    l7.value = null; l7.known = false;                                 /* 지금 상태 모름 */
    l7[TR_KEY] = { r: true, value: 90, age_ms: 120000 };               /* 마지막 사건은 남아 있다 */
    await evaluate(client, inject(ST(M7)));
    await sleep(160);
    const mv7 = await evaluate(client, READM);
    console.log('  · r vs 비트 / known:false → ' + JSON.stringify({ A1: mv7.A1, LD: mv7.LD }));
    ok('🔴 분모: 두 행에 전이 문장이 그려졌다',
       !!(mv7.A1 && mv7.A1.meas && mv7.LD && mv7.LD.meas), JSON.stringify([mv7.A1, mv7.LD]));
    ok('🔴 사건 낱말을 **`r` 에서** 읽는다 (비트=찼다인데 r=false → "비었다")',
       !!(mv7.A1 && /비었다/.test(mv7.A1.meas || '') && !/찼다/.test(mv7.A1.meas || '')),
       JSON.stringify(mv7.A1) + ' — 비트에서 읽으면 "찼다" 가 나온다. 그게 거짓말이다');
    ok('🔴 `known:false` 인데 전이가 있으면 **지우지 않는다** (LD → 90 이 보인다)',
       !!(mv7.LD && /90/.test(mv7.LD.meas || '')), JSON.stringify(mv7.LD));
    ok('🔴 그때 **`(마지막)` 표지가 붙는다** — 옛 사건을 현재로 읽지 않게',
       !!(mv7.LD && /\(마지막\)/.test(mv7.LD.meas || '')), JSON.stringify(mv7.LD));
    ok('반대로 `known:true` 면 그 표지가 **없다** (A1)',
       !!(mv7.A1 && !/\(마지막\)/.test(mv7.A1.meas || '')), JSON.stringify(mv7.A1));

    /* 🔴 음성 대조 ③ — **"모름" 과 "모듈이 죽었다" 가 갈리나** (루트가 확인을 요구했다)
       근거: 등(`zlamp`)과 값 기호가 **`known` 하나**에서 갈린다 →
         known:true  = occ 가 오고 있다(살아 있다) → 등 ● · 기호 ●/○
         known:false = 아무것도 안 온다            → 등 ○ · 기호 ⏱
       🔑 측정값 칸의 **문구는 둘 다 같다.** 갈리는 것은 **행의 다른 두 칸**이다. */
    const M4 = BASE.map((m) => Object.assign({}, m));
    M4.find((x) => x.name === 'B1').known = false;      /* 죽은 모듈 */
    await evaluate(client, inject(ST(M4)));
    await sleep(160);
    const live = await evaluate(client, `(() => {
      ...
      for (const li of document.querySelectorAll('#zone-detail .zmod')) {
        ...
        const mm = t.match(/\(([A-Z0-9]{2})\)/);
        const el = li.querySelector('.zmod__tr');
        out[mm ? mm[1] : t.slice(0, 6)] = {
          lamp: (li.querySelector('.zlamp') || {}).dataset ? li.querySelector('.zlamp').dataset.state : null,
          meas: el ? el.textContent : null };
      }
      return out;
    })()`);
    console.log('  · 죽은 모듈 대조 → ' + JSON.stringify({ A1: live.A1, B1: live.B1 }));
    ok('🔴 살아 있으나 값 없음(A1)과 죽은 모듈(B1)이 **등에서 갈린다**',
       !!(live.A1 && live.A1.lamp === 'ok' && live.B1 && live.B1.lamp === 'unknown'),
       JSON.stringify([live.A1, live.B1]));
    ok('🔑 그런데 측정값 **문구는 둘이 같다** — 그 구분은 등이 나른다(값으로 기록)',
       !!(live.A1 && live.B1 && live.A1.meas === live.B1.meas), JSON.stringify([live.A1, live.B1]));
  }
