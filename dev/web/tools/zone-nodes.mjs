...
try {
  ...
  for (const zid of visible(Object.keys(expect))) {
    ...
    for (const dv of want) {
      ...
      /* ⚠ **정확히 같은지가 아니라 보이는지**를 잰다 — 2026-08-25 에 사용자가 표기를 확정했다:
         *"아두이노 박스 안에 모듈 아이디"* → 머리글이 `P1` 에서 **`아두이노 P1`** 이 됐다.
         🔑 이 검사의 뜻은 *"머리에 노드 ID 가 명시된다"* 이지 *"머리가 곧 ID 다"* 가 아니다 —
         `=== dv` 로 잠가 두면 **표기를 개선할 때마다 계측기가 빨강**이 된다(화면은 멀쩡한데). */
      ok(zid + '/' + dv + ' 박스 머리에 노드 ID 가 명시된다', box.headText.includes(dv), '머리글: ' + box.headText);
    }
  }
  ...
  /* ── 🔴 `occupied:false` 를 "비었다" 로 그리지 않는다 (socket 확정 · wsjson.h:255-259) ──
     서버 점유는 **아는 센서들의 OR** 이라 **전부 모르면 그 값도 `false`** 다. `bool` 이 그 구분을
     못 나르므로 서버가 `value_state` 로 따로 말한다. 🔴 이 조건은 실제로 생긴다(B1 은 상시 모름).
     🔑 검사는 **세 갈래를 다 밟는다** — known / unknown / partial. 하나만 재면 나머지가 조용히 깨진다. */
  {
    const VS = { type: 'map', srv_id: 'T-1', epoch: 21, grid: { rows: 1, cols: 3 },
      zones: [
        { id: 'V1', kind: 'parking', cells: [[0, 0]], active: { ok: true, reason: null },
          modules: [{ devid: 'P1', name: 'A1', kind: 'IP', idx: 0 }] },
        { id: 'V2', kind: 'parking', cells: [[0, 1]], active: { ok: true, reason: null },
          modules: [{ devid: 'P1', name: 'B1', kind: 'IP', idx: 1 }] },
        { id: 'V3', kind: 'parking', cells: [[0, 2]], active: { ok: true, reason: null },
          modules: [{ devid: 'P1', name: 'C1', kind: 'IP', idx: 2 }] },
      ] };
    await evaluate(client, inject(VS));
    await evaluate(client, inject({ type: 'state', srv_id: 'T-1', epoch: 21, ts_ms: 9, zones: [
      /* 전부 안다 · 비었다 → 초록 "빈 자리" 가 맞다(대조군) */
      { id: 'V1', occupied: false, reserved: false, value_state: 'known', value_known: 1, value_total: 1, actions: {}, modules: [] },
      /* 🔴 전부 모른다 → occupied:false 지만 **"비었다" 가 아니다** */
      { id: 'V2', occupied: false, reserved: false, value_state: 'unknown', value_known: 0, value_total: 2, actions: {}, modules: [] },
      /* 일부만 안다 → 색은 서버 판정을 따르고 문구가 모르는 칸을 말한다 */
      { id: 'V3', occupied: false, reserved: false, value_state: 'partial', value_known: 1, value_total: 2, actions: {}, modules: [] },
    ] }));
    await sleep(160);
    /* 🔑 **헬퍼를 쓴다** — 손으로 순회하면 클릭이 원소를 떨어뜨리는 그 함정에 다시 빠진다.
       (내가 그 주석을 써 놓고 이 블록에서 다시 밟았다. 그래서 `cdp.mjs` 의 `ZONE_HELPERS` 로 옮겼다.) */
    await evaluate(client, ZONE_HELPERS);
    const vs = await evaluate(client, `__eachZone((c, p) => ({
      view: c.dataset.view || null,
      sum: (p.querySelector('.zone__sum') || {}).textContent || null,
      aria: c.getAttribute('aria-label'),
    }))`);
    console.log('  · value_state → ' + JSON.stringify(vs));
    ok('분모: 세 자리가 다 그려졌다', Object.keys(vs || {}).length === 3, JSON.stringify(vs));
    ok('대조군: 전부 아는데 비었으면 "빈 자리" 초록이다 (V1)',
       !!(vs.V1 && vs.V1.view === 'free' && vs.V1.sum === '빈 자리'), JSON.stringify(vs.V1));
    ok('🔴 전부 모르면 "빈 자리" 라고 **말하지 않는다** (V2 → 점유 모름)',
       !!(vs.V2 && vs.V2.sum === '점유 모름'), JSON.stringify(vs.V2));
    ok('🔴 전부 모르면 **초록으로 칠하지 않는다** (V2 → unknown)',
       !!(vs.V2 && vs.V2.view === 'unknown'), JSON.stringify(vs.V2));
    ok('일부만 알면 색은 유지하고 **문구가 모르는 칸을 말한다** (V3)',
       !!(vs.V3 && vs.V3.view === 'free' && /센서 1개 모름/.test(vs.V3.sum || '')), JSON.stringify(vs.V3));
    ok('접근 이름에도 그 사실이 실린다 (V2)',
       !!(vs.V2 && /점유 모름/.test(vs.V2.aria || '')), JSON.stringify(vs.V2 && vs.V2.aria));
  }
