...
try {
  ...
  /* 🔴 **이 검사기는 지형을 박고 있다** — 절마다 자리 하나를 독점해서 서로 간섭하지 않게 만들었고,
     그 이름을 `A1~A5` · `B1~B5` 로 **하드코딩**했다(옛 지형: 두 열 10칸).
     지금 쓰는 화면은 자리가 다섯이라(A1~A5) `B*` 가 없고, 그러면 `readTile` 이 전부 `null` 을 낸다.
     ⚠ 그 빨강은 **화면 결함이 아니라 검사기가 낡은 것**이다 — 큐 계약 자체는 화면에 살아 있다.
     ✅ 그래서 빨강 대신 **왜 못 재는지**를 말하고 멈춘다. 거짓 빨강은 거짓 초록만큼 나쁘다.
     📖 고치려면: 자리 이름을 화면에서 읽고 절마다 배정하되, 자리가 모자라면 절 시작에
        그 자리의 pending 을 비워 재사용해야 한다. 그건 별도 작업이다. */
  const NEED = ['A1','A2','A3','A4','A5','B1','B2','B3','B4','B5'];
  const HAVE = await evaluate(client, `[...document.querySelectorAll('.tile[data-slot]')].map(t => t.dataset.slot)`);
  const MISSING = NEED.filter((x) => HAVE.indexOf(x) < 0);
  if (MISSING.length) {
    console.log('\n  ⏭ **이 검사기는 옛 지형(A·B 두 열 · 10칸)을 가정한다.**');
    console.log('     화면의 자리: ' + JSON.stringify(HAVE));
    console.log('     검사기가 쓰는데 없는 자리: ' + JSON.stringify(MISSING));
    console.log('     🔑 이것은 **화면 결함이 아니라 검사기가 낡은 것**이다 — 큐 계약은 화면에 있다.');
    console.log('        (`grep -c "queued" <화면>` 으로 그 경로가 사는지 따로 셀 수 있다)');
    console.log('     ✅ 거짓 빨강을 내지 않으려고 여기서 멈춘다. 옛 화면을 재려면 `--file <경로>`.');
    console.log('\n' + '─'.repeat(60));
    console.log('  미측정 — 지형 불일치로 못 쟀다 (' + pass + ' pass 까지 확인)\n');
