...
if (MODE === 'deploy') {
  ...
  /* 🔴🔴 **배포 단계가 없는 구조에서는 배포하지 마라** (2026-08-27 신설)
     서머리 구조에서는 서버가 **자기 폴더의 `index.html` 을 직접 읽는다** —
     그 파일이 곧 우리가 고치는 파일이라 **복사할 것이 없다.**
     ⚠ 그런데 절차 문서들은 옛 구조(`~/parking-bin`)를 그대로 말하고 있다.
     🔑 문서가 낡는 것은 못 막는다. **도구가 스스로 말하게 하는 것**이 처방이다 —
       읽는 사람이 문서를 의심할 필요가 없어진다.
     ★ 판별자 : **대상 파일이 이 저장소 안에 있나.** 안에 있으면 원본과 같은 실체다. */
  /* 🔴 **경로를 정규화하고 비교한다.** `target` 이 상대경로(`서머리/server/index.html`)로 오면
     날 문자열 비교가 빗나가 **막지 못한다** — 2026-08-27 에 실제로 그렇게 통과했고
     저장소 파일에 표지를 찍고 `.prev-*` 백업까지 만들었다.
     ⚠ 그리고 그 `.prev-*` 는 `서머리/server/**` 규칙으로 **socket 소유**라 web 이 지우지도 못한다 —
       이 파일 아래 주석이 경고하던 바로 그것이다. **경고를 적어 놓고 내가 밟았다.** */
  const repo = resolve(process.cwd());
  const abs = resolve(target);
  const inRepo = abs === repo || abs.startsWith(repo + '/');
  if (inRepo && !has('--force')) {
    console.log('  🔴 배포 대상이 **이 저장소 안**이다: ' + target);
    console.log('     → 서버가 그 파일을 직접 읽는 구조라 **배포 단계가 없다.**');
    console.log('       파일을 고치면 새로고침만으로 반영된다. 복사도 표지 찍기도 필요 없다.');
    console.log('     ✅ 지금 확인하는 법 : node web/tools/deploy-screen.mjs --check --port <포트>');
    console.log('        (서버 cwd 의 파일과 저장소 원본이 **같은지** 그 명령이 값으로 답한다)');
    console.log('     ⚠ 옛 구조(~/parking-bin 으로 복사)로 정말 돌아간 것이면 --force 를 붙여라.');
    process.exit(2);
  }
  ...
  /* 직전 판을 남긴다 — socket 이 바이너리에 하는 것과 같은 규율(srv_parking.prev-*).
     ...
     그때 `.prev-*` 는 안전망이 아니라 **쓰레기**다. 특히 dev_server 폴더는
     **기여자에게 압축으로 나가는 폴더**라 그 안의 백업 파일이 그대로 배포된다.
     ⚠ 그리고 `.prev-*` 의 소유권은 dev_server 폴더 규칙(socket)으로 떨어져 **web 이 지우지도 못한다.**
     🔑 `~/parking-bin` 처럼 **git 밖 경로**에는 쓰지 마라 — 거기서는 백업이 유일한 되돌림이다.
     ⚠⚠ 이 주석에 glob 을 쓰지 마라: 별별슬래시가 **블록 주석을 조기 종료한다.**
         2026-08-20 에 그렇게 깨뜨렸고 `node --check` 는 **통과했다**(나머지가 우연히 유효한 JS 였다). */
  if (has('--no-backup')) {
