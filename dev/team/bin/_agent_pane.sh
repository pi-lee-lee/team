...
# 정의 파일 머리(frontmatter)에 `model: <id>` 가 있으면 그 모델로 띄운다. 없으면 기본 모델.
#   검토 전용 에이전트처럼 **역할마다 다른 모델**이 필요할 때 쓴다. 정의 파일이 유일한 출처다.
model="$(sed -n '2,/^---$/p' "$AGENTS_DIR/$name.md" | sed -n 's/^model:[[:space:]]*//p' | head -1)"
