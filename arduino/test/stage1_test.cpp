...
int main() {
  ...
  printf("\n[47] 히스테리시스 — 경계에서 진동하지 않는가\n");
  {
    ...
    //   🔑 **히스테리시스는 값이 있을 때만 작동한다.** 이 갈래는 값이 없어서 그 밖이다.
    {
      ...
      // 🔴🔴 **실기 경로로 읽는다 — 매 루프 여러 번 부른다.**
      //   옛 시험은 `readSensor` 를 **1회씩** 불러서 `N` 이 "루프 수" 인 결함을 **못 봤다.**
      //   실기(`tick()` → `readSensors()`)는 슬롯당 수백~수천 번 부른다.
      //   monitor 가 실측으로 잡았다: 해제 직전 연속 빈칸 `V` 가 **76%(16/21)가 1개**였다.
      //   🔑 §"시험 경로 ≠ 실기 경로" — **7개 시험이 다 통과했는데 실기가 안 돌았다.**
      //   ⚠ 그래서 아래는 **한 슬롯 안에서 여러 번** 부르고, 슬롯은 `g_millis` 로 넘긴다.
      // 🔑 **한 슬롯을 넘기고, 그 안에서 여러 루프를 돈다.**
      //   실기의 루프는 시간이 거의 안 흐르므로 루프마다 `g_millis` 를 올리지 않는다 —
      //   올리면 게이트(200ms)와 슬롯 경계가 섞여 무엇을 재는지 흐려진다.
      auto slotOnce = [&]() -> uint8_t {
        g_millis += 1300;                          // 슬롯(1200)과 게이트(200)를 확실히 넘긴다
        uint8_t r = 0;
        for (uint8_t q = 0; q < 50; q++) r = node.readSensor(0);   // 같은 슬롯 안 50루프
        return r;
      };
      // ① 🔴 **못 쟀다 1슬롯(=50 루프)은 참는다**
      g_pulseIn = 0;
      ok(slotOnce() == 1,   "★★★ 못 쟀다 **1슬롯(50루프)** → 유지 (루프 수로 안 센다)");
      // ② 2슬롯도 참는다 — 🔑 슬롯을 넘기려면 SLOT_MS(1200ms) 를 넘겨야 한다
      g_millis += 1200;
      ok(slotOnce() == 1,   "★★★ 못 쟀다 2슬롯 → 여전히 유지 (N=3 미만)");
      // ③ 🔴 3슬롯에 해제한다
      ok(slotOnce() == 0,   "★★★ 못 쟀다 **3슬롯** → 해제 (N=3 × 1.2초 = 3.6초)");
      ok(node.noRead[0] == 3, "★★ `noRead` 가 정확히 3 이다 (루프 수가 아니다)");

      // ④ 🔴 **값이 오면 계수가 0 으로 돌아간다** — 안 돌아가면 언젠가 무조건 해제된다
      node.occMask = 1; node.noRead[0] = 0;
      ok(readAt(50) == 1,   "★ 값이 오면 다시 찼다");
      g_pulseIn = 0;
      ok(slotOnce() == 1,   "★★★ 값 뒤 못 쟀다 1슬롯 → 유지 (**계수가 0 으로 되돌았다**)");
