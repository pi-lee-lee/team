...
//
// 2초의 근거:
//   · 실측에서 `SEND OK` 는 **같은 초 안에** 돌아온다(`monitor/serial-newbase.log` 정상 구간).
//     2초는 그 2배 이상이라 정상 왕복을 자르지 않는다.
//   · 기전 B 탐지가 **약 5초**로 묶인다 — 살아있음 불변식(10초)보다 확실히 짧아
//     그쪽이 먼저 발동해 이 안전망이 무의미해지는 일이 없다.
//   ⚠ 이 값은 **아직 실측 근거가 얇다**(정상 `SEND OK` 지연 분포를 따로 재지 않았다).
//     A 창 8시간 자료에서 분포가 나오면 그때 근거를 붙여 다듬어라.
