...
#include <map>
#include <deque>      // 카메라 요청 대장 링(shots.h) — 앞에서 빼고 뒤에 넣는다
...
// 🔴 `runtime.h` **뒤** 여야 한다 — `epoch_ms()`·`instance_tag()` 를 쓴다.
//   ⚠ 앞에 두면 "선언되지 않은 식별자" 로 깨진다. 순서가 곧 의존이다.
#include "shots.h"     // 카메라 pull — 요청번호 대장(REQ-0318 · SPEC-camera-pull.md)
#include "node.h"
#include "zone.h"
...
struct Pending {             // 아두이노에 내려보내고 ACK 를 기다리는 요청
    ...
    // 🔴 **이 명령이 갈 장치.** `rid` 는 서버 전역에서 유일하지만 **소켓은 노드마다 따로다** —
    //   이 칸이 없으면 배출이 아무 노드에나 보내고, 받은 보드에 그 모듈이 없어 **조용히 버려진다.**
    //   ⚠ 그리고 ACK 대조에도 쓴다 : 같은 `rid` 의 ACK 이 **다른 보드에서** 오면 그것은 짝이 아니다.
    std::string devid;
...
//     `ParkingServer` 를 만들지 않는 경로도 **소켓쌍을 쓰므로 `SIGPIPE` 가 필요하다.**
...
// 🔴 v2 — `sensor()`/`actuator()` 를 `module()` 하나로 합쳤다.
...
Measure ParkingLot::measure(const std::string& devid, const std::string& name) {
    // 🔑 **같은 (devid,name) 은 덮어쓴다** — `control`·`label` 과 같은 규칙이다.
    //   두 벌이 생기면 어느 수치로 판정하는지 아무도 모른다.
    for (size_t i = 0; i < measures_.size(); i++)
        if (measures_[i].devid == devid && measures_[i].name == name)
            return Measure(this, i);
    MeasureDecl m; m.devid = devid; m.name = name;
    measures_.push_back(m);
    return Measure(this, measures_.size() - 1);
}
// ⚠ 세 setter 는 **값을 검사하지 않는다.** 검사는 `MeasureDecl::complete()` 한 곳이다 —
//   여기서도 막으면 규칙이 두 곳에 생기고, 갈리면 관대한 쪽이 이긴다.
//   🔑 그리고 연쇄 중간에는 아직 불완전한 것이 정상이다(`.near(10)` 만 부른 순간).
Measure& Measure::near(long cm) { lot_->measures_[idx_].near_cm = cm; return *this; }
Measure& Measure::far (long cm) { lot_->measures_[idx_].far_cm  = cm; return *this; }
Measure& Measure::min (long cm) { lot_->measures_[idx_].min_cm  = cm; return *this; }

Control& Control::toggle(const std::string& on, const std::string& off) {
    ControlDecl& c = lot_->controls_[idx_];
    c.widget = ControlDecl::TOGGLE; c.on_label = on; c.off_label = off; return *this;
}
...
// 🔑 **한 줄 전달이다.** 판정 규칙은 `nodes.h::module_sensor_state` 하나뿐이고,
//   자리 판정도 같은 것을 쓴다 — 래퍼에 규칙을 두면 그 순간 판정자가 둘이 된다.
SensorReading ParkingServer::sensorReading(const std::string& devid,
                                           const std::string& moduleName) const {
    return p_->srv.module_reading(devid, moduleName);
}
...
void ParkingServer::onManualPlate(ManualPlateFn fn) { p_->srv.manual_cb_ = fn; }
void ParkingServer::onSlotPick(SlotPickFn fn) { p_->srv.pick_cb_ = fn; }
// 🔑 **한 줄 전달이다.** 끊긴 연결을 걷는 규칙은 `state.h::chooser_count()` 한 곳에 있다.
int ParkingServer::chooserCount() const { return p_->srv.chooser_count(); }
bool ParkingServer::chooserEnabled() const { return g_chooser_on; }
// 🔑 **그대로 싣는다.** 엔진은 `phase` 의 뜻을 모르고, 알 필요도 없다 —
//   흐름은 `lot.cpp` 의 것이고 여기서 해석하면 규칙이 두 곳이 된다.
void ParkingServer::entryStatus(const EntryStatus& e) { p_->srv.entry_ = e; }
void ParkingServer::slotPlate(const std::string& spotId, const std::string& plate,
                              const std::string& source) {
    const int i = p_->srv.slot_index(spotId);
    if (i < 0) { logf("!", "번호판 기록 실패 — 그런 자리가 없다: " + spotId); return; }
    p_->srv.slots[i].plate = plate;
    p_->srv.slots[i].plate_source = plate.empty() ? std::string() : source;
    // 🔴 **기록은 즉시 남긴다.** 다음 스냅샷을 기다리면 그 사이 서버가 죽었을 때
    //   *"차는 섰는데 번호가 없는"* 자리가 디스크에 남는다.
    p_->srv.write_log_if_changed();
}
// 🔑 표지 `▸` 는 **기여자 줄**이다 — 엔진 줄(`=` · `!` · `←ARD` …)과 grep 으로 갈린다
void ParkingServer::log(const std::string& msg) const { logf("▸", msg); }
long long ParkingServer::cameraShoot() { return p_->srv.shot_issue(); }
// 🔑 넷 다 **한 줄 전달**이다 — 판정은 엔진(`persist.h`)에 있다.
//   래퍼에 로직을 두면 자가검증이 못 닿는다.
std::string ParkingServer::cameraPlate(long long shotId) const { return p_->srv.shot_plate(shotId); }
int         ParkingServer::cameraState(long long shotId) const { return p_->srv.shot_state(shotId); }
std::string ParkingServer::cameraReason(long long shotId) const { return p_->srv.shot_reason(shotId); }
bool        ParkingServer::cameraCancel(long long shotId, const std::string& why) {
    return p_->srv.shot_cancel(shotId, why);
}
long long   ParkingServer::cameraAge(long long shotId)   const { return p_->srv.shot_age(shotId); }
bool        ParkingServer::phoneOnline() const { return !p_->srv.phones.empty(); }
long long   ParkingServer::phoneEpoch()  const { return p_->srv.phone_epoch_; }
bool ParkingServer::deviceReady(const std::string& devid) const {
    const Node* n = p_->srv.node_by_devid(devid);
    // 🔴 **등록만으로는 부족하다 — 지금 살아 있어야 한다.** 끊긴 보드도 `reg_done` 은 참으로
    //   남아 있어서, 그것만 보면 **죽은 보드에 명령을 내고 `true` 를 돌려받는다.**
    return n && n->reg_done && p_->srv.node_online(*n);
}
bool ParkingServer::parkingSpotAvailable(const std::string& spotId) const {
    const int i = p_->srv.slot_index(spotId);
    if (i < 0 || p_->srv.slots[i].occupied || p_->srv.slots[i].reserved) return false;
    // 🔴 **센서를 신뢰할 수 없는 자리는 "쓸 수 있다"가 아니다**.
    //   `occupied == 0` 은 *"비었다"* 가 아니라 **낡은 값**일 수 있다 — 센서가 죽었으면
    //   그 자리에 차가 들어와도 서버가 모르고, 그 자리는 **영구 점유로 굳는다.**
    return p_->srv.zone_usable(spotId);
}
bool ParkingServer::moduleReady(const std::string& devid, const std::string& moduleName) const {
    const Node* n = p_->srv.node_by_devid(devid);
    if (!n || !n->reg_done || !p_->srv.node_online(*n)) return false;
    // 🔑 **그 보드가 그 모듈을 실제로 등록했나.** 이름만 맞고 보드가 다르면 명령이 사라진다.
    for (size_t k = 0; k < n->mods.size(); k++)
        if (n->mods[k].first == moduleName) return true;
    return false;
}
