// downlink.h — 아두이노 하행(서버 → 장치) 발행·재전송·회수. `struct Server` 의 몸통 조각 — server.cpp:185
// ⚠ 단독 컴파일 불가 · include 자리가 곧 문법이다 · 📖 server.cpp "목차"
    ...
    void dispatch(char kind, sock_t ws_fd, const std::string& brid,
        ...
        // 🔴 **어느 보드로 갈지 여기서 정한다** — 자리에 결속된 센서의 노드다.
        //   ⚠ 못 찾으면 **주 노드로 떨어뜨리지 않는다.** 그 폴백이 있으면 P2 의 자리 명령이
        //     조용히 P1 으로 가고, P1 은 모르는 자리라 무시한다 — **아무 데도 오류가 안 남는다.**
        Node* target = zone_owner(slot);
        if (!target) {
            logf("!", std::string("하행 발행 포기 — 자리 ") + slot
                      + " 에 결속된 센서 노드가 없다 (kind=" + std::string(1, kind) + ")");
            rid_release(rid);
            return;
        }
        Pending p;
    ...
    void dispatch_test(sock_t ws_fd, const std::string& brid,
        ...
        Pending p;
        p.wire_rid = rid; p.ws_fd = ws_fd; p.browser_rid = brid;
        // 🔴 자리를 지목한 `T` 는 그 자리의 노드로 간다. **자리 없는 `T`(`"??"`)만 주 노드로 간다** —
        //   ⚠ 이건 **새로 만든 폴백이 아니라 옛 거동을 그대로 둔 것**이다. 시험 무장은 아직
        //     서버 전역 개념이라 보낼 곳이 하나뿐이다. 🔑 그 전역이 노드별이 되면 이 갈래는 사라진다.
        {
            Node* tgt = (slot != "??") ? zone_owner(slot) : &park;
            if (!tgt) {
                logf("!", "T 발행 포기 — 자리 " + slot + " 에 결속된 센서 노드가 없다");
                rid_release(rid);
                return;
            }
            p.devid = tgt->devid;
    ...
    void dispatch_sim(sock_t ws_fd, const std::string& brid) {
        ...
        Pending p;
        p.wire_rid = rid; p.ws_fd = ws_fd; p.browser_rid = brid;
        // ⚠ 시뮬레이터는 아직 **주 노드 전용**이다(장치 쪽 `M` 이 그 노드에만 있다). 옛 거동 그대로다.
        p.devid = park.devid;
        p.kind = 'M'; p.top = 0;
        p.sent_ms = now_ms(); p.tries = 1;
        pend[rid] = p;
    ...
    // ── 🔴 자리 조작 `G` — **전선 형식은 arduino 의 파서에서 읽어 맞췄다**
    //
    //   `G,<rid>,<idx>,<op>,<ck>`
    ...
    // ── 🔴 ②-d — **그 모듈을 등록한 노드를 찾는다**
    //   ⚠ `state_json()` 이 **주 노드만** 보면 나머지가 전부 `known:false` 로 나간다.
    ...
    // 🔑 **하행은 노드를 고쳐 쓴다**(소켓·큐·세션 상태). 그래서 non-const 판이 따로 필요하다.
    Node* node_by_devid(const std::string& dev) {
        if (dev == park.devid) return &park;
    ...
    void notify_cmd(const Pending& p, CmdResult::Kind k, int devResult) {
        ...
        if (p.web_cmd && p.ws_fd != BAD_SOCK && conns.count(p.ws_fd)) {
            ...
            o << "{\"type\":\"cmd_result\",\"rid\":" << jstr(p.browser_rid)
              // 🔴 **명령을 실제로 받은 보드다.** 주 노드를 고정으로 실으면 —
              //   P2 로 나간 명령의 결과가 **`P1` 이름으로 화면에 올라간다.**
              //   ⚠ 그 봉투는 형식이 멀쩡해서 화면도 사람도 못 알아챈다.
              << ",\"devid\":" << jstr(p.devid)
              << ",\"module\":" << jstr(p.slot)
              << ",\"value\":" << p.g_arg
              << ",\"outcome\":\"" << out << "\""
        ...
        CmdResult r;
        r.kind = k; r.devid = p.devid; r.module = p.slot;   // 🔑 기여자 콜백도 실제 보드를 받는다
        r.value = p.g_arg; r.deviceResult = devResult; r.rid = p.wire_rid;
        cmd_cb_(r);
    }

    // ═══════ 🔴 판정 수치 하행 `K` (SPEC-sensor-value.md §15.4) ═══════════════
    //
    // 기여자가 `lot.measure("P1","A1").near(10).far(20).min(5)` 로 선언하면
    // **등록 직후** 서버가 장치에 내려보낸다. → ✅ 수치를 고치는 데 **굽기가 필요 없다.**
    //
    // 🔴 **장치에 저장하지 않는다** — 매 등록 때 다시 보낸다. 낡은 값이 원리적으로 안 남는다.
    // 🔴 **`G` 보다 먼저 나간다** — 판정 수치가 없는 채로 명령을 받으면 장치가 옛 기준으로 답한다.
    //
    // ⚠ **한 창 상한이 `min(DEV_RX_RING_B, DOWN_BATCH_CAP_B)` 다**(지금 64B).
    //   그래서 한꺼번에 안 쏘고 **창마다 한 묶음씩** 뺀다 — `k=3` 이면 1창, `k=6` 이면 2창.
    //   🔑 이 상한을 무시하고 다 쏘면 링을 넘겨 **세션이 끊긴다**.
    void queue_measure_config(Node& n) {
        // 🔑 **모든 노드에 보낸다.** 하행이 노드별 소켓(`send_node`)으로 갈렸으므로
        //   *"주 노드에만"* 이라는 옛 제약은 사라졌다. 그 제약이 남아 있으면 둘째 보드의 모듈은
        //   **장치 컴파일 기본값으로 판정**하는데, 그건 조용해서 아무도 안 본다.
        std::vector<std::string>& q = cfg_q_[n.devid];
        q.clear();
        if (!lot_) return;
        const std::vector<MeasureDecl>& ms = lot_->measures();
        for (size_t i = 0; i < n.mods.size() && i < (size_t)REG_MODS_MAX; i++) {
            for (size_t j = 0; j < ms.size(); j++) {
                if (ms[j].name != n.mods[i].first) continue;
                // 🔑 `devid` 가 비었으면 **아무 장치나** — 조립 표의 1인자 형태와 같은 규칙이다.
                if (!ms[j].devid.empty() && ms[j].devid != n.devid) continue;
                if (!ms[j].complete()) {
                    // 🔴 **막지 않고 말한다** — 증상이 보이는 오류이기 때문이다(장치가 옛 값을 쓴다).
                    //   ⚠ 다만 그 증상은 *조용하므로* 로그가 **무엇을 고쳐야 하는지**까지 말한다.
                    cfg_skipped_++;
                    logf("!", "판정 수치를 안 보낸다 — " + n.devid + "/" + ms[j].name
                              + " : " + ms[j].why_incomplete()
                              + " (장치의 컴파일 기본값이 그대로 쓰인다)");
                    break;
                }
                char b[64];
                snprintf(b, sizeof(b), "K,%d,%03lX,%03lX,%03lX,",
                         (int)i, ms[j].near_cm, ms[j].far_cm, ms[j].min_cm);
                q.push_back(build_line(b));
                break;
            }
        }
        if (!q.empty()) {
            char b[160];
            // 🔴 **"큐에 넣었다" 이지 "보냈다" 가 아니다.** 실제 전송은 다음 창이고,
            //   그 사이 링크가 죽으면 **이 수만큼 안 나간다.**
            //   🔑 monitor 가 이 줄을 합산해 장치 `kapply` 와 대조하므로 **낱말을 정확히 쓴다** —
            //     "내려보낸다" 로 적었더니 그쪽이 *보낸 수* 로 읽었다.
            //   ★ 실제로 나간 수는 아래 `flush_measure_config()` 가 따로 찍는다.
            snprintf(b, sizeof(b), "판정 수치 %zu건을 큐에 넣었다 — device=%s (창 상한 %dB · %zu창 예상 · 전송은 다음 창)",
                     q.size(), n.devid.c_str(), cfg_cap_b(),
                     (q.size() * 20 + (size_t)cfg_cap_b() - 1) / (size_t)cfg_cap_b());
            logf("=", b);
        }
    }
    // 🔑 **리터럴로 박지 않는다** — `--down-cap` 이 돌면 같이 움직여야 한다.
    int cfg_cap_b() const {
        return (DEV_RX_RING_B < DOWN_BATCH_CAP_B) ? DEV_RX_RING_B : DOWN_BATCH_CAP_B;
    }
    // 한 창에 담을 수 있는 만큼만 뺀다. **남은 것은 버리지 않고 다음 창으로 미룬다.**
    void flush_measure_config(Node& node) {
        if (node.fd == BAD_SOCK) return;
        std::map<std::string, std::vector<std::string> >::iterator it = cfg_q_.find(node.devid);
        if (it == cfg_q_.end() || it->second.empty()) return;
        std::vector<std::string>& q = it->second;
        const size_t cap = (size_t)cfg_cap_b();
        std::string payload;
        size_t sent = 0;
        while (sent < q.size() && payload.size() + q[sent].size() <= cap) {
            payload += q[sent]; sent++;
        }
        // 🔴 한 줄도 못 담으면 **상한이 한 줄보다 작다는 뜻**이다 — 조용히 영원히 대기하지 않는다.
        if (sent == 0) {
            logf("!", "🔴 판정 수치 한 줄이 창 상한(" + std::to_string(cap)
                      + "B)보다 크다 — 내려보낼 수 없다. 버린다 (device=" + node.devid + ")");
            q.clear();
            return;
        }
        send_node(node, payload);
        cfg_sent_ += (long long)sent;
        q.erase(q.begin(), q.begin() + (long)sent);
        // 🔴 **나간 수를 따로 찍는다.** `→ARD` 줄은 한 덩어리에 여러 `K` 를 담아
        //   **줄 수를 세면 틀린다**(첫 줄에만 접두가 붙는다).
        //   🔑 이 줄이 장치 `kapply` 와 대조할 **유일하게 정확한 수**다.
        {
            char sb[128];
            snprintf(sb, sizeof(sb), "판정 수치 %zu건 나갔다 — device=%s (남은 %zu건)",
                     sent, node.devid.c_str(), q.size());
            logf("=", sb);
        }
        // ⚠ 남았으면 **몇 창 걸렸는지** 센다 — `설정미도달` 이 요약에서 이것을 읽는다.
        if (!q.empty()) cfg_gap_slots_++;
    }
    // 이 노드가 아직 못 받은 판정 수치가 있나
    bool cfg_pending_for(const std::string& devid) const {
        std::map<std::string, std::vector<std::string> >::const_iterator it = cfg_q_.find(devid);
        return it != cfg_q_.end() && !it->second.empty();
    }
    size_t cfg_pending_total() const {
        size_t n = 0;
        for (std::map<std::string, std::vector<std::string> >::const_iterator it = cfg_q_.begin();
             it != cfg_q_.end(); ++it) n += it->second.size();
        return n;
    }
    ...
    // 🔴🔴 **모듈에 값을 보낸다** — 공개 API `ParkingServer::send()` 의 실물
    //
    //   ⚠ 하행 경로를 **차단봉 전용**으로 두면(`dispatch_gate`, 0/1) 기여자가 자기 모듈에
    ...
    bool send_to_module(const std::string& devid, const std::string& name, long arg,
                        ...
                        sock_t ws_fd = BAD_SOCK, const std::string& brid = "") {
        // 🔑 **`(devid, name)` 복합키로 찾는다.** `devid` 가 비면 이름으로만 찾되
        //   **둘 이상이면 거절한다**(`resolve_module` 이 그 갈래를 갖고 있다) —
        //   아무 보드나 고르면 그 순간부터 어느 보드가 받았는지 아무도 모른다.
        Node* n = resolve_module(devid, name);
        if (!n) {
            logf("!", "발행 실패 — 모듈 `" + (devid.empty() ? name : devid + "." + name)
                      + "` 의 소유 노드를 못 찾았다. "
                      "**그 장치가 등록한 이름과 맞춰라**(대소문자까지)");
            return false;
        }
        // 🔴 **안 붙은 노드에는 발행하지 않는다.** 큐에 넣으면 배출마다 실패하고,
        //   화면은 그동안 *"진행 중"* 에 머문다 — 실패가 보이는 편이 낫다.
        if (n->fd == BAD_SOCK) {
            logf("!", "발행 실패 — 노드 `" + n->devid + "` 가 지금 안 붙어 있다 (모듈 " + name + ")");
            return false;
        }
        int idx = -1;
        ...
        if (idx < 0) {
            logf("!", "발행 실패 — 노드 `" + n->devid + "` 에 모듈 `" + name + "` 이 없다. "
                      "**그 장치가 등록한 이름과 맞춰라**(대소문자까지)");
            return false;
        }
        ...
        //   ⚠ 이 사실을 **주석에만** 두지 마라 — 주석을 고치면 같이 사라진다. 시험이 정본이다.
        p.wire_rid = rid; p.ws_fd = ws_fd; p.browser_rid = brid;
        p.web_cmd = (ws_fd != BAD_SOCK);
        p.devid = n->devid;          // 🔑 **찾아낸 노드로 못 박는다** — 인자가 비었어도 여기서 확정된다
    ...
    void tick() {
        ...
        // 🔑 **노드마다 따로 묻는다.** 등록은 노드별 상태이므로 주 노드만 물으면
        //   나머지 보드는 **영영 미등록으로 남고**, 미등록이면 그 보드의 비트열을 못 푼다.
        {
            std::vector<Node*> qns = all_nodes();
            for (size_t qi = 0; qi < qns.size(); qi++) {
                Node& qn = *qns[qi];
                if (qn.fd == BAD_SOCK || !qn.seen || qn.reg_done || qn.reg_giveup) continue;
                if (!qn.reg_first_ms || (t - qn.reg_first_ms) <= REG_TIMEOUT_MS) continue;
                if ((t - qn.last_q_ms) < DOWN_SLOT_MS) continue;
                if (qn.q_sent >= REG_Q_MAX) {
                    qn.reg_giveup = true;
                    reg_giveups++;
                    ...
                    qn.q_sent++; qn.last_q_ms = t; reg_qsent++;
                    // `Q,<ck>` — **인자 없음.** 주소는 소켓이 갖고 있고, 이 시점의 devid 는
                    // 서버가 확신할 수 없는 값이다(미등록이라 묻는 것이다 · 설계 §5).
                    std::string q = build_line("Q,");
                    if (!send_raw(qn.fd, q.data(), q.size(), qn.devid.c_str()))
                        node_send_failed(qn, "Q 송신 실패");
                    else logf("→ARD", qn.devid + " Q (등록 질의 " + std::to_string(qn.q_sent) + "/"
        ...
        // 🔑 **창 포기도 노드별이다.** 전역으로 재면 조용한 보드 하나 때문에 **말짱한 보드의
        //   창까지 포기**하고, 그 탐침이 남의 큐를 실어 나른다.
        {
            std::vector<Node*> dns = all_nodes();
            for (size_t di = 0; di < dns.size(); di++) {
                Node& dn = *dns[di];
                if (dn.fd == BAD_SOCK) continue;
                if ((t - dn.last_ms) <= DOWN_DMAX_MS) continue;
                if (!dn.dmax_armed || (t - dn.last_dmax_ms) < DOWN_SLOT_MS) continue;
                bool has_q = false;
                for (size_t k = 0; k < downq.size(); k++)
                    if (downq[k].devid == dn.devid) { has_q = true; break; }
                if (!has_q) continue;
                dn.last_dmax_ms = t;
                dn.dmax_armed = false;     // 장치가 한 줄이라도 말할 때까지 다시 안 쏜다
                dmax_flushes++;            // 그래서 이 수가 "몇 번 포기했나"로 읽힌다
                // ⚠ **탐침 크기로 쏜다**(4건 아님) — 증거가 없는 자리다.
                flush_downq(dn, "S 가 안 온다 — 창 포기(탐침)", true, DOWN_PROBE_N);
            }
        }
        ...
        for (std::map<uint16_t, Pending>::iterator it = pend.begin(); it != pend.end(); ++it) {
            ...
            //   → 3회 소진이 최악 ≈6.9초. web 타이머는 `expires_ms(≤4800) + 6000`
    ...
    // ---------- 아두이노 재부팅 → 재동기화 (§7.4)
    void resync_reservations(const char* why) { resync_reservations_for(0, why); }
    // 🔑 **`only` 가 있으면 그 노드가 맡은 자리만 재하달한다.**
    //   ⚠ 전부 내리면 보드 하나가 등록할 때마다 **멀쩡한 보드의 창까지 예약으로 채운다** —
    //     보드가 늘수록 그 낭비가 곱으로 는다. 그리고 그 창은 진짜 명령이 기다리는 창이다.
    void resync_reservations_for(const Node* only, const char* why) {
        resync_count++;                      // 오탐 증명용 계수기 (§7.4)
        std::vector<int> live;
        for (int i = 0; i < 10; i++) {
            if (!slots[i].reserved) continue;
            if (only && zone_owner(SLOT_ID[i]) != only) continue;
            live.push_back(i);
        }
        logf("⟳", std::string("재동기화(") + why + ") — 살아 있는 예약 "
                  + std::to_string(live.size()) + "건 재하달"
                  + (only ? (" · device=" + only->devid) : std::string()));
