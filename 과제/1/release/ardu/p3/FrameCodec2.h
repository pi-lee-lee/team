...
//
// ⚠ **왜 `2` 가 있나**: 이 파일의 코드가 `Modules.h` 를 필요로 해서 그 뒤에 와야 했다.
//   모듈 표가 스케치(`pN.ino`) 머리로 올라간 지금 **그 제약은 사라졌다** — 합칠 수 있다.
//   ⚠ 다만 합치면 코드가 옮겨지므로 `hex` 가 달라진다. **굽기 경계가 필요한 정리다.**
//
// 순서: `FrameCodec.h` → `Modules.h` → **이 파일**
// ═════════════════════════════════════════════════════════════════════════
...
static bool sendAck(uint16_t rid, char s0, char s1, uint8_t result) {
  // ✏️ 여기서 프레임을 만들던 코드를 **뺐다.** 배치가 캐시에서 재생성하므로
  ...
  //
  // 🔑 그리고 **유실 경로가 오히려 준다**: 🔴 "즉시 보내고 실패하면 push" 라
...
// ─────────────────────────────────────────────────────────────────────────
// 🔴 슬롯 배치 — **S 프레임 + 밀린 ACK 를 한 거래로 묶는다**
//
