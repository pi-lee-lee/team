...
// 🔴 **명령 인자용 32비트 파서** — LCD 7자리 숫자 같은 값이 이 통로로 온다.
...
static void processTest(char* f[], char* s0, char* s1, uint8_t* result) {
  ...
  *s1 = '?';
  ...
  if (top == 'A' || top == 'D') {
    ...
    //   멈출 것도 재개할 것도 없다. "무장 중 시뮬 정지" 는 여기 없다.
    *result = 0;
    return;
  }
...
static void processCommand(char* cand) {
  ...
  if (idx == 0xFF) {
    ...
    // 판정: slot ::= ("A"/"B")("1".."5") / "??". 고정 표식을 쓰고 ACK 를 생략하지 않는다.
    ...
    // 🔴 **예약이 옳은지는 장치가 안 따진다 — 서버가 정한다.**
    //   장치는 받은 R/C 를 자기 비트에 **그대로 반영**하고, 반영할 수 있었는지만 ACK 한다.
    if (type == 'R') {
      node.resMask |= bit;
      result = 0;
    } else {                                            // 'C' — 취소
      node.resMask &= (uint16_t)~bit;
      result = 0;
    }
  }
...
#include "EspLink_at.h"   // ← AT 응답 어휘 해석. **위치를 옮기지 마라**
