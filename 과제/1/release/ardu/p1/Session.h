...
// ─────────────────────────────────────────────────────────────────────────
// 주기 처리
#include "EspLink_ladder.h"   // ← EspLink 링크 계층. **위치를 옮기지 마라**

// ✏️ `sensorTick()` 은 **`ParkingNode::readSensors()` 에 있다.**
...
static void statusTick(unsigned long now) {
  ...
  //     그 결함이 주기를 1.000s 가 아니라 **1.113s** 로 만들었다).
  ...
  //
  // 🔴 **"창 안에서 시작한다"로는 부족하다. 창 안에서 *끝나야* 한다.**
  ...
  static const uint16_t SLOT_TX_RESERVE_MS = 200;   // 최악 배치 상한(185ms)에 여유를 더한 값
  ...
  // ⚠ `debounced` 는 **더 이상 전송을 트리거하지 않는다**(슬롯이 주기를 정한다).
  ...
  // ⚠ `changeAt`(위)과 **같은 함정이고 같은 관용구로 막는다.** 실기에서 터지는 자리다:
  ...
  // REQ-0501 — 이번 S 의 내용이 마지막 에코와 다를 때만 시리얼에 찍는다(상행은 그대로 1Hz)
  txEchoWanted = (occSnap != echoOcc) || (resSnap != echoRes) || (tmaskSnap != echoTmask);
  const uint16_t okPrev = lastSendOkMs;          // 직전 슬롯의 SEND OK 지연 — 이번 것은 아직 안 왔다
  bool ok = sendSlotBatch(&batchAcks, &batchBytes);
  if (ok) { cntTx++; if (txEchoWanted) { echoOcc = occSnap; echoRes = resSnap; echoTmask = tmaskSnap; } }
  ...
  //     monitor 파서가 우리 산문 안의 `SEND OK` 를 세어 `0/7` 을 낼 수 있다.
  ...
  Serial.print(F(" okprev="));                              // REQ-0501 — 직전 CIPSEND→SEND OK 지연(ms). [TX]/[AT] 없이도 잰다
  if      (okPrev == OKPREV_NA) Serial.print(F("NA"));      //   NA = 부팅 뒤 아직 한 번도 안 쟀다
  else if (okPrev == OKPREV_TO) Serial.print(F("TO"));      //   TO = 직전 대기가 시한 포기/소켓 복구로 끝났다
  else if (awaitingSendOk)      Serial.print(F("W"));       //   W  = 직전 것이 아직 대기 중이다(값이 아니라 상태)
  else                          Serial.print(okPrev);
