#pragma once
// ═════════════════════════════════════════════════════════════════════════
// EspLink_ladder.h — 🔒 **링크 계층**: `espInit` · `espReset`(사다리 구동) — 링크 재수립
// ═════════════════════════════════════════════════════════════════════════
//
...
// 🔴🔴 **이것은 거동 변경이다. 다음 굽기 항목이고 자기 축을 갖는다**
//
...
static void espReset(unsigned long now) {
  ...
  switch (sent) {
    case NET_CWJAP:
      // 답이 없으면 그냥 다시 묻는다. **전진하지 않는다** — 그게 이 고장의 원인이었다.
      netStep = NET_CWJAP;
      break;
    ...
    case NET_CIFSR:
      ...
      if (++cifsrTries >= 3) {
        // ★ 소진 **사유**를 먼저 읽는다. 아래 리셋보다 앞이어야 한다(§8.2-12)
        const bool refused = cifsrRefused;
        cifsrTries = 0;
        cifsrRefused = false;
        // ★ 판별자 ② — **응답이 가비지여도 성립한다.** 18:48:12 리셋에서 CIFSR 응답이
        //   통째로 깨져 `0.0.0.0` 문자열이 안 나왔고, 판별자 ① 만으로는 놓친다.
    ...
    case NET_CIPCLOSE:
      ...
      if (closeAttempts >= CLOSE_ATTEMPT_LIMIT) {
        ...
        // ★ 로컬 명령이 세 번 연속 안 먹은 것은 **AT 계층이 잠긴 것**이다.
