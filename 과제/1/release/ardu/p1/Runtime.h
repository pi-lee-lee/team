...
//   ⚙ 응용 성질 — 잠금 대상이 아니다
//
// 🔓 **기여자는 이 파일을 열지 않는다.** 스케치(`pN.ino`) 의 `setup()`/`loop()` 가 여기로 위임한다.
...
// ═════════════════════════════════════════════════════════════════════════
// 🔴🔴 **위치를 옮기지 마라.** 스케치(`pN.ino`) 의 **정해진 자리에서**(목록 맨 끝) `#include` 된다.
...
void ParkingNode::begin() {
  ...
  randomSeed((unsigned long)analogRead(A2) ^ micros()); //A1 Default
