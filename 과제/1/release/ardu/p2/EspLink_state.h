#pragma once
// ═════════════════════════════════════════════════════════════════════════
// EspLink_state.h — 🔒 **링크 계층**: 링크 상태 · 복구 사다리 · `netAdvance` · `netSendStep`
// ═════════════════════════════════════════════════════════════════════════
//
...
//   이 파일은 스케치(`pN.ino`) 의 **원래 그 자리에서** `#include` 된다.
//   전처리 결과 텍스트가 같으면 산출물이 같고, 그래서 이 분리는 **굽기 축이 아니다**.
//   ⚠ 다른 자리로 옮기거나 여러 헤더의 순서를 바꾸면 **그 순간 거동 변경이 된다** —
//     원장 §19(`espInit`: 흩어진 것을 모으면 그것이 곧 거동 변경이다)가 그 선례다.
//
// ⚠ **`arduino/client/EspLink.h`(08-16)와 다른 것이다.** 그쪽은 추출 실험이고
...
// **타이머 구동에서 응답 구동으로.**
//
// 🔴 **각 단계는 응답이 올 때까지 기다린다. 타이머로 전진하지 마라.**
//   타이머만 보고 전진하면 앞 명령이 끝나기 전에 다음 것을 쏘게 되고, 그것들이
//   **전부 `busy p...` 로 거부된다** — 그런데 거부는 조용하다(`OK` 가 없을 뿐이다).
//   📖 그 상태의 실기 로그: docs/arduino/LEDGER.md
//
// 알아 둘 것 셋:
...
//   ESP-AT v2.2 문서와 어휘가 다르다. 확인된 차이:
...
// ─────────────────────────────────────────────────────────────────────────
enum {
  NET_RST = 0, NET_CWMODE,
  NET_SLEEP,                          // 🔴 AT+SLEEP=0 — 절전(modem-sleep)을 끈다. CWMODE 뒤·CWJAP 앞. RST 뒤마다 다시 보낸다(저장 안 됨)
  NET_CWJAP,
  NET_CIFSR,                          // ★ IP 를 실제로 받았는지 물어본다
  NET_CIPMUX, NET_CIPSTART,
  NET_CIPCLOSE,                       // 복구 전용 — 낡은 소켓을 닫는다
  ...
  NET_CWLAP,                          // 주변 AP 목록
  // ⓘ `AT+CWJAP?`(질의형) 단계를 넣었다가 뺐다 — **이 펌웨어는 `ERROR` 로 답한다.**
  //   미지원 명령을 매 실패마다 쏘면 로그만 더럽힌다. 같은 질문(결합했는가)은
  //   `AT+CIFSR` 이 실주소를 답하는지로 더 확실하게 답할 수 있다.
  NET_CWQAP,                          // ★ 2단 — 명시적 결합 해제 후 다시 붙는다
  NET_STEP_COUNT
};
// 각 단계의 **응답 대기 상한**. 넘으면 그 단계를 다시 시도한다(전진이 아니다).
//   CWJAP 30000 : 이 명령이 **16초까지 걸린다** + 여유. 🔴 짧게 잡으면 응답 전에 포기한다
//   CWLAP  9000 : 스캔은 채널을 훑으므로 오래 걸린다
//   CWQAP  3000 : 모듈 로컬 동작이라 왕복이 없다. 3초면 넉넉하다
static const uint16_t NET_WAIT[] = { 2500, 800, 800, 30000, 2500, 1200, 8000, 800, 1500, 1500, 9000, 3000 };
...
static bool    netHasIp = false;      // ★ CIPSTART 의 전제조건
...
// ═════════════════════════════════════════════════════════════════════════
// 복구 사다리 — **같은 자극을 반복하는 것은 사다리가 아니다**
//
// 🔴 `CWJAP FAIL` 은 **"AP 에 못 붙었다" 가 아닐 수 있다.** IP 할당까지는 매번 성공하고
//   **약 12초 뒤 결합이 소실되는** 경우가 있다. 그러면 같은 CWJAP 를 다시 쏘는 것은
//   **같은 12초를 다시 만드는 것**이다 — 재시도 횟수를 늘려도 원리적으로 안 낫는다. **단을 올려야 한다.**
//   📖 그렇게 18분을 못 빠져나온 실기 기록: docs/arduino/LEDGER.md
//
// 원칙 셋:
...
//   단 4  ESP 하드웨어 리셋선            — 배선 필요(ESP_RST_WIRED). 여기 들어온다
//   단 5  ESP 전원 재투입                — **부품 미보유로 미구현.** 아래 applyRung() 참조
//   단 6  AVR 워치독                     — 파일 상단 ENABLE_WDT. 여기 들어온다
//
...
// 이게 없으면 **응답이 아예 없는 CWJAP** 가 30초마다 조용히
...
// (배선 금지목록과 모듈 표의 핀 칸을 그대로 대조했다. 10칸을 다 쓰는 최대 구성 기준):
//   D0,D1 USB시리얼 / D2~D6 센서 5칸 / D7,D8 ESP 시리얼 / D9~D12 센서 4칸 /
//   D13 온보드LED / A0 센서 1칸 / A1 난수시드 / A4,A5 I2C 예약
...
// ═════════════════════════════════════════════════════════════════════════
// 🔴 **이 값은 *물리 배선 상태* 다. 코드로 검증할 수 없다.**
//   `0` 은 **"아직 안 물렸다" 가 아니라 "물릴 필요가 없다"** 다 — 사용자가 설계로 확정했다.
//   ⚠ **바꾸려면 사용자에게 다시 물어라.** 문서·기억·"물려 있을 것이다" 는 근거가 아니다.
//   📖 사용자 원문과 그 판단의 근거: docs/arduino/LEDGER.md
// ═════════════════════════════════════════════════════════════════════════
#define ESP_RST_WIRED 0   // 0 = 리셋선 없음(설계 확정)
//   🔴 **1 로 두면 오히려 나쁘다.** `applyRung()` 이 `espRstAssert()` 로 가는데 선이 없으면
//     **아무 일도 안 일어나고 `AT+RST` 로도 안 빠진다** — 4·5단이 백오프만 쓰고 **조치는 0** 이 된다.
//     그리고 부팅 로그가 `배선됨(A2)` 이라고 **거짓을 말한다.**
//     ★ **표지를 지우는 변경은 그 표지가 지키던 진실을 같이 지운다.**
//   🔮 선을 실제로 물리면 그때 1 로 올린다. **물리기 전까지 0 이 참이다.**
...
// 무엇을 보냈는지 찍는다. 이게 없으면 5초마다 CIPSTART 를 재시도하는지조차
// 로그로 확인할 수 없다.
#if DEBUG
...
static void netSendStep(uint8_t s) {
  switch (s) {
    ...
    case NET_SLEEP:   wifi.print(F("AT+SLEEP=0\r\n"));  DBG_NET("1s", "SLEEP=0 (절전 끔)"); break;
  ...
  // ★ CWJAP 의 "답이 오지 않는 경우"를 실패로 세기 위한 표식(위 cwjapPending 주석).
