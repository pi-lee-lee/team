...
// ★ 아래 셋은 `feedRxChar()` 보다 **앞에 있어야 한다**(그 안에서 쓴다).
...
// ★ `SEND OK` 바이트 흐름 매칭 상태(1B)와 그 경로로 잡은 수.
...
// 🔴 **`penddrop=0` 을 읽을 수 있게 만드는 분모와 대역.**
//
...
// 🔴 **숫자를 박지 마라 — 링 크기에서 유도한다.** 박아 두면 링을 바꿨을 때
//   계수 문턱만 옛 값으로 남아 **진단이 거짓말한다**(값과 진단이 갈리는 형태).
static const uint8_t RXBUF_THRESHOLD = _SSB_MAX_RX_BUFF;   // 기전 경계이지 표본 기준이 아니다

// 🔴 **줄의 첫 바이트가 도착한 시점의 슬롯 오프셋.**
//
...
//   반증: 서버 재전송이 난 구간에서 `oow` 가 0 이었다(socket, 2차 주입).
//
