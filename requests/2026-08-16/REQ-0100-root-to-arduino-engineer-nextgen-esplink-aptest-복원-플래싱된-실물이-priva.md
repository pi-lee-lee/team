...
> # ⚠ 이 아래의 hex 대조는 **2026-08-16 16:0x 기준이며 지금은 무효다** (REQ-0109 로 추가)
>
> 그 시점에는 참이었다. 그러나 **이후 사용자가 `조별과제샘플/client.ino`(EspLink 이전 판)로
> 재플래싱했다.** 그래서 **지금 칩에 도는 것은 여기서 대조한 EspLink 판이 아니다.**
>
> **증명이 틀린 것이 아니라 비교 대상이 교체된 것이다.** 아래 내용은 "복원본이 그 시점의
> 실장본과 동일했다"는 기록으로서 그대로 유효하며, 복원본의 무결성 근거로는 계속 쓸 수 있다.
> 다만 **"칩에 있는 것 = EspLink 판"이라는 뜻으로 인용하면 틀린다.**
> 현재 칩 상태는 `arduino/INTERVENTIONS.md` 맨 위를 봐라.
