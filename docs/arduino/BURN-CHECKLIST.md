...
> ## 🔴🔴 **두 가지를 빼야 한다 — 부트로더 영역과, 빌드가 안 쓴 자리**
>
> ### ② **빌드가 안 쓴 자리(`0xFF`)를 비교에 넣으면 작은 판에서 항상 MISMATCH 가 난다**
> `avrdude` 는 **빌드가 쓴 페이지만** 쓴다. 새 판이 옛 판보다 작으면 그 뒤 영역에
> **옛 코드가 그대로 남는다** — 지워지지 않는다. 실행에 영향 없는 **도달 불가 코드**다.
> **실측(2026-08-20)**: 484B 작은 판을 굽자 응용영역 전체로는 **1,176 바이트 상이**,
> 빌드가 쓴 자리만 보면 **0**. 잔재 1,176 중 485 가 직전 판과 같았다(나머지는 더 옛 층).
> → **묻는 것은 "칩에 내가 올린 것이 있는가"이지 "칩이 내 파일과 같은가"가 아니다.**
> ⚠ 항상 발화하는 검사는 아무 말도 안 하는 검사다. 아래 스크립트가 그 축으로 고쳐져 있다.
>
> ### ① 그리고 부트로더 영역도 빼야 한다
>
...
# ★★ **빌드가 실제로 쓴 자리만 본다** (2026-08-20 에 이것 때문에 헛 MISMATCH 가 났다)
wrote = [i for i in range(APP_END) if b[i] != 0xFF]
diff  = [i for i in wrote if a[i] != b[i]]
left  = sum(1 for i in range(APP_END) if b[i] == 0xFF and a[i] != 0xFF)

print('빌드가 쓴 바이트:', len(wrote))
print('그 자리에서 다른 바이트:', len(diff))
print('IDENTICAL' if not diff else 'MISMATCH 첫 위치 0x%04X' % diff[0])
print('참고 — 안 쓴 자리에 남은 옛 코드 잔재:', left, 'B (도달 불가. 정상이다)')
PY
```
...
## 5-b. 🔴 **부팅 로그를 같은 포트 세션에서 뜬다** — 안 뜨면 그 판의 표지는 영영 못 본다

**부팅 로그는 그 부팅에만 있다.** 나중에 뜨려면 포트를 다시 잡아야 하고 **그것이 리셋을 하나 더
만든다** — 관측 창이 이미 열려 있으면 그 창을 자른다.
🔑 **그래서 굽기·판독과 **같은 포트 세션 안에서** 한 번 떠 둔다. 개입이 늘지 않는다.**

```bash
python3 arduino/.burn/cap.py "$PORT" 12 > arduino/.burn/boot-$(date +%H%M%S).txt
```
⚠ **읽을 때 `grep -a` 를 써라** — 캡처에 제어문자가 섞여 `grep` 이 바이너리로 판정하면
매치를 **조용히** 안 찍는다(빈 출력을 "없다"로 읽게 된다).

**개입 0 으로 재는 몫도 있다** — 컴파일 상수로 결정되는 표지는 `.elf` 문자열로 확인된다:
```bash
python3 - <<'PY'
b = open('arduino/.burn/out/client.ino.elf','rb').read()
for t in ['있어야 하는 문구', '❌ 0건이어야 하는 문구']:
    print(t, b.count(t.encode('utf-8')))
PY
```
🔴 **`strings` 를 쓰지 마라 — 한글 UTF-8 을 안 뽑는다.** 바이트로 직접 센다.
🔴 **문구를 화면에 보이는 줄로 적으면 못 찾는다** — 코드가 `F("[REG] 등록 ") + F("전송 ")` 처럼
**조각으로** 만든다. `Serial.print` 조각 단위로 적어라.
⚠ **`-flto` 는 안 쓰는 갈래의 문자열을 지운다**(예: 훅 미등록이면 `훅` 0건).
**문자열 부재를 기능 부재로 읽지 마라.**
...
**굽고 나서 무엇을 볼지 정하면 이미 늦다.** 보이는 것에 맞춰 기준이 흔들린다.
그리고 ⚠ **"없어야 하는 것"을 같이 적어라** — 안 적으면 없는 것을 아무도 안 센다(원장 §42).

**배너 한 줄이 1차 출처다.** 자리·센서·모듈 **셋을 따로** 찍는다:

```
[PARKING NODE] proto v1 / <자리> spots / <센서> sensors / <모듈> modules / dev=<id>
```
| 값 | 어디서 오나 | 무엇과 비교하나 |
|---|---|---|
| `spots` | `SLOT_ROWS` = `SLOT_N`/2 | **서버 조립 표의 `spot` 수** |
| `sensors` | `SLOT_N` | 모듈 표의 `I*` 줄 수 |
| `modules` | `moduleCount()` | `D,*,<drain>,<n>` 의 **`n`** |

⚠ **셋은 다른 값이다.** 옛 배너는 `SLOT_N` 하나만 `slots` 로 찍어 **센서 수를 자리 수로 읽게**
만들었다(원장 §48). **하나만 찍으면 반드시 오독된다.**

**2차(독립) 확인**
```
[REG] 등록 전송 <크기>B      ← 크기 = **10 + 11×모듈수** (이름 2글자·kind 2글자일 때)
D,*,7,<n>,                   ← n = 모듈 수
D,<이름>,<kind>,  × n 줄     ← 이름과 kind 를 **눈으로** 대조한다
S 의 자리 필드 폭 = ceil(n/4)
```
🔑 **모듈 수가 배너·`D,*` 머리·`D,` 줄 수 **세 각도**에서 같은 값이어야 한다.**

**❌ 없어야 하는 것도 적는다** (예)
```
VIRTUAL_MODULES=0 으로 구웠으면 → `D,E1,` `D,X1,` **0건**
옛 구성의 값 (예: `12 modules`) **0건**
```

**조건부 표지는 그렇게 표시해라** (원장 §52)
```
무조건 : `[G] idx=<n> arg=<v>`  — 모든 G 프레임이 탄다. **1차로 줘라**
조건부 : `[LC] <값>`            — **LC 가 지목될 때만** 나온다. 부재를 실패로 읽으면 안 된다
```
