...
> ✅ **이 `cmp` 가드는 음성 시험을 통과했다**(2026-08-19 · 원장 §32) — 스테이징 헤더를 일부러
> 낡게 만들었더니 **그 파일만** 짚었다. **`git status` 줄도 비ASCII 경로에서 실제로 뜬다.**
> ⚠ **다만 git 출력의 경로는 `"\354\241\260…"` 로 이스케이프돼 보인다.** 지금 블록은
> **경로를 읽지 않고 "비었는가"만 보므로 안전하다.** 🔴 **나중에 이 출력을 파싱하도록 고치면
> 그 순간 함정에 빠진다** — 그때는 `-z` 와 `core.quotepath=false` 를 같이 써라.
>
