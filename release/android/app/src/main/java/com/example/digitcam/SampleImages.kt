...
object SampleImages {
     ...
     * 🔑 값은 [GrayConvert] 가 갖는다 — 촬영본 경로와 **같은 수여야** 두 경로가 비교된다.
     */
    // 🔑 상수가 아니라 **지금 값**을 읽는다 — 메모리 압력에 따라 내려갈 수 있다(REQ-0407).
    private val MAX_EDGE get() = GrayConvert.activeMaxEdge
     ...
     * 이미지로 인정하는 확장자.
     *
     * 🔴 **`.jpeg` 가 빠져 있으면 그 파일은 "실패" 가 아니라 *목록에 없다*.** 카카오톡·갤러리에서
     * 나온 사진은 `.jpeg` 이고, 그것을 assets 에 넣어도 아래 분기가 **디렉터리로 취급**해
     * 조용히 건너뛴다. 그러면 화면에도 로그에도 아무것도 안 나오고, 그 침묵이
     * *"인식이 안 된다"* 와 **같은 모양**이다(`0/0` 과 `0/15` 를 가르는 자리다).
     */
    private val IMAGE_EXTS = listOf(".png", ".jpg", ".jpeg")
    ...
    private fun listDir(context: Context, path: String, prefix: String): List<String> = try {
        ...
        buildList {
            for (e in entries) {
                if (IMAGE_EXTS.none { e.endsWith(it, ignoreCase = true) }) {
    ...
    fun load(context: Context, name: String): Frame? {
        ...
        // 🔑 휘도 변환은 [GrayConvert] 하나에만 둔다 — 촬영본 경로와 갈리면
        // "샘플로는 되는데 촬영본으로는 안 된다" 를 디버깅할 수 없다.
    ...
    private fun logGrayStats(name: String, gray: ByteArray, w: Int, h: Int) {
        ...
        Log.i(TAG, "샘플 '$name' ${w}x$h · gray mean=${st.mean} min=${st.min} max=${st.max}")
    }

    private fun downscale(src: Bitmap): Bitmap {
        val t = GrayConvert.scaledSize(src.width, src.height, MAX_EDGE)
        if (t.width == src.width && t.height == src.height) return src
        return Bitmap.createScaledBitmap(src, t.width, t.height, true)
    }
}
