// seam.h — 이음매: 디바이스 → 도메인 . `struct Server` 의 몸통 조각 — server.cpp:183
// ⚠ 단독 컴파일 불가 · include 자리가 곧 문법이다 · 📖 server.cpp "목차"
    ...
    // §4-B (에서 web 과 합의한 계약) — **"받았고 아직 안 보냈다"**
    //
    ...
    void send_queued(const DownQ& q) {
        ...
        o << "{\"type\":\"queued\",\"rid\":" << jstr(q.brid)
          ...
          << ",\"ahead\":" << ahead
          << ",\"expires_ms\":" << left
          // 🔴 **이탈 후 예산**. `expires_ms` 는 큐 대기까지만 덮으므로
    ...
    //   새로운 문장을 못 싣는다. 봉투 형식은 똑같아서 화면이 갈라 볼 것이 없다.
    void ack_msg(sock_t fd, const std::string& rid, const std::string& slot,
                 const std::string& message) {
        std::ostringstream o;
        o << "{\"type\":\"ack\",\"rid\":" << (rid.empty() ? std::string("null") : jstr(rid))
          << ",\"slot\":" << (slot.empty() ? std::string("null") : jstr(slot))
          << ",\"result\":0,\"message\":" << jstr(message) << "}";
        if (fd != BAD_SOCK && conns.count(fd)) ws_send(fd, o.str());
    }
    ...
    void send_ack(sock_t fd, const std::string& rid, const std::string& slot, int result,
        ...
        // 🔑 수동 입력은 **전선 왕복이 없다** — 서버가 번호를 받은 것이 전부다.
        //   ⚠ 기본 문장("예약되었습니다")이 나가면 **하지 않은 일을 했다고 말한다.**
        else if (kind == 'P') m = "번호를 접수했습니다";
