# 주차 서버 — Visual Studio 판 (`릴리즈/VS_server`)

```
Windows(VS)   ../server.slnx 을 열고 F5              →  콘솔 창이 뜬다
macOS/Linux   c++ -std=c++11 -O2 -o srv server.cpp lot.cpp   →  ./srv
화면          http://127.0.0.1:9900/
```

## 🔴 이 폴더의 소스는 **정본의 사본**이다 (2026-08-27 갱신)

```
정본 : `서머리/server`      ← 여기가 바뀌면 **이쪽도 같이** 바꿔야 한다
사본 : `릴리즈/VS_server/server`
✅ 2026-08-27 기준 **소스 34개 바이트 동일**(확인함)
```
> ### ⚠ **두 곳이 갈리면 VS 에서 여는 사람이 옛 서버를 돌린다** — 그리고 **아무 오류도 안 난다.**
> ### 🔑 검산 : `diff -r` 로 `.h`/`.cpp` 를 대라. **문서를 믿지 말고 값을 봐라.**

## 🔴🔴 **빌드는 검증되지 않았다** — 우리는 macOS 에서 작업한다

```
🔴 **MSVC 가 없다.** 다만 **텍스트로만** 은 아니다 — 아래 §"윈도우 갈래를 실제로 파싱한다" 를 봐라
   ✅ 소스 34개 정본과 동일
   ✅ `.vcxproj` 파일 목록 정합 — 헤더 31 · cpp 3 · **빠진 것 0**
   ✅ `.vcxproj` ↔ `.filters` 어긋남 **0** · **컴파일 대상**도 맥 빌드와 같다
   ✅ **`_WIN32` 갈래를 컴파일러에 태운다** — `windows.h` 매크로를 켠 채로 (2026-08-27)
🔴 **"빌드된다" 고 말하지 않는다.** 윈도우에서 처음 여는 사람이 그것을 확인한다
```

## ✅ 윈도우 갈래를 **실제로 파싱한다** — 그리고 그 검사에 이빨이 있다 (2026-08-27)

```bash
cd 릴리즈/VS_server/server
c++ -std=c++20 -fsyntax-only -Wall -Wextra -Wreorder -Wsign-compare \
    -I ../../../cpp/winparse/shim -I . \
    -Wno-macro-redefined -Wno-unknown-pragmas \
    -include ../../../net/probe/win_shim_extra.h \
    ../../../cpp/winparse/win_parse.cpp
```
`cpp/winparse/` 는 `_WIN32` 를 켜고 이 트리를 파싱시키는 하니스다. shim 이 진짜 `<windows.h>` 처럼
**`near`·`far`·`IN`·`OUT` 을 빈 매크로로** 정의하기 때문에, 그 함정이 여기서 드러난다.

### 🔴🔴 **초록불이 "안 봤다" 가 아니라는 증거 — 같은 명령이 샘플 트리에서는 빨강이다**

```
릴리즈/VS_server/server          →  ✅ exit=0 · 오류 0 · 경고 0
조별과제샘플/VS_server/server     →  🔴 exit=1 · 오류 **6**
     parking.h:149  Measure& near(long cm);   ← `near` 매크로에 먹혔다
     parking.h:150  Measure& far(long cm);
     server.cpp:313·314  Measure::near / Measure::far
```
> ### ★★ **그러니 "샘플이 검증된 판이다" 는 틀렸다.**
> ### 샘플은 `near`/`far` 고침이 **들어가기 전 판**이고, **윈도우에서 컴파일되지 않는다.**
> ### 🔑 **샘플에서 윈도우 대응을 베껴 오면 이 결함이 되살아난다.**

⚠ **이것은 MSVC 가 아니다.** clang 이 `_WIN32` 갈래를 파싱한 것이고, shim 은 진짜 SDK 가 아니다.
  ✅ 정확한 뜻 : *"컴파일 단계에서 **볼 수 있는 것**에는 없다."*  ❌ 아닌 뜻 : *"빌드된다."*

### ⚠ shim 에 없는 것은 `net/probe/win_shim_extra.h` 로 채운다
```
runtime.h:298  `_fullpath`  →  shim 에 없다(cpp 소유라 그쪽을 안 고쳤다)
시그니처는 Microsoft Learn 으로 대조 : char *_fullpath(char*, const char*, size_t)  <stdlib.h>
```
### ⚠ 깨질 만한 자리 — **오류가 나면 여기부터 봐라**
```
✅ **확인한 것**(macOS 에서 텍스트로 잰 것)
```
🔵 `ws2_32.lib` 링크 — **소스가 스스로 건다** : `server.cpp:67  #pragma comment(lib, "ws2_32.lib")`
   ★ 그래서 `.vcxproj` 에 링크 설정이 **없어도 된다**(실제로 없다 · grep 0)
🔵 `/utf-8` — `.vcxproj` 에 **있다**(5곳). 한글 소스가 깨지지 않는다
🔵 파일 목록 — 헤더 31 · cpp 3 · **빠진 것 0** · `.filters` 와 어긋남 **0**
```
### ✅ MSVC 이식성 정적 점검 — **전수로 셌다**(34개 소스)
| 항목 | 건수 | |
|---|---|---|
| POSIX 헤더가 **`#ifdef` 밖**에 있는 곳 | **0** | 🔵 열한 종(`sys/socket`·`unistd`·`fcntl`…) 전부 분기 안 |
| `__attribute__` | **0** | 🔵 GCC/Clang 전용 문법 없음 |
| `strcasecmp` · `strdup` · `ssize_t` | **0** | 🔵 POSIX 전용 이름 없음 |
| 지정 초기화자(`= { .x = 1 }`) | **0** | 🔵 C++ 에서 MSVC 가 싫어하는 것 없음 |
| 가변길이 배열(VLA) | **0** | 🔵 |
| `close(fd)` 를 소켓에 직접 | **0** | 🔵 `closesock()` 로 감싸 뒀다 |
| `usleep`·`nanosleep` | **0** | 🔵 **`sleep` 을 안 쓰는 구조**(설계) |
| `%zu` | **28** | 🔵 전부 `snprintf` 안. **MSVC 2015+ 지원**(v145 이면 문제없다) |

🔴 **못 확인한 것**(윈도우에서 처음 여는 사람이 볼 것)
```
[1] ~~`_CRT_SECURE_NO_WARNINGS` 가 **없다**(grep 0)~~ → 🔴 **거짓이었다. `server.cpp:33` 에 있다**
    ★ 내가 **`.vcxproj` 만 세고 소스에는 안 댔다.** `grep 0` 은 참이었는데 **grep 한 곳이 틀렸다**
    🔑 §"`0` 이 나오면 **내 자가 이걸 잴 수 있나** 를 먼저 물어라" — 그 `0` 은 *"없다"* 가 아니라
      *"내가 딴 데를 봤다"* 였다. **거짓 `0` 은 없는 결함을 쫓게 한다**
    🔵 그래서 이 항목은 **위험이 아니다** — 소스가 스스로 켠다
[2] `#include` 순서 — `winsock2.h` 는 `server.cpp` 의 `_WIN32` 갈래 안에 있다.
    ⚠ 다른 헤더가 `windows.h` 를 먼저 끌어오면 충돌한다. **그때 오류 문구에 `WinSock` 이 뜬다**
[3] 🔵 **표준은 문제없다** — `.vcxproj:121  <LanguageStandard>stdcpp20</LanguageStandard>`.
    소스는 C++11 로 쓰였고 `server.cpp:11` 이 `/std:c++14` 를 권한다. **C++20 이 그보다 높다**
    ⚠ 다만 **툴셋이 `v145`** 다(`.vcxproj:74`) — **그 버전이 설치돼 있어야** 열린다.
    ★ 없으면 VS 가 *"플랫폼 도구 집합을 찾을 수 없다"* 고 한다 → **프로젝트 우클릭 → 대상 변경**
[4] 🔴 **화면이 404** — 아래 `--web-root` 절을 봐라. **서버는 그래도 뜬다**
```

## ✅ 화면 — **이 폴더에 셋 다 있다. 인자도 이미 박혀 있다**
```
index.html (409,110B) · user8080.html (35,413B) · user8081.html (35,476B)   ← 정본과 **바이트 동일**
```
```xml
server.vcxproj 끝에 :
  <LocalDebuggerCommandArguments>--web-root=$(ProjectDir) --data-dir=$(ProjectDir) --no-chooser</LocalDebuggerCommandArguments>
```
> ### 🔵 **F5 만 누르면 된다.** 속성 창에서 따로 칠 것이 없다.
```
🔴 **왜 인자가 필요한가** : 서버는 화면 뿌리를 **`cwd` 로 안 정한다**(2026-08-27 구조 변경).
   기본값이 `<실행파일>/../web` 인데 VS 산출물은 `x64/Debug/` 에 떨어져 **그 기본값이 안 맞는다**
   → 인자가 없으면 `! 🔴 정적 뿌리 … index.html 이 없다` + **화면 404**.
     🔵 그래도 **서버 기능(아두이노·폰·차단봉)은 돈다** — 그건 설계다
⚠ **`.vcxproj.user` 에 두지 않았다** — 그 파일은 저장소에 안 들어가서 **다른 사람이 열면 인자 없이 돈다**
⚠ `--no-chooser` 는 **확정 구성의 일부**다 — 빼면 8081 선택 갈래가 살아나 **차단봉이 먼저 열린다**
```
🔴 **화면 셋도 정본의 사본이다** — `서머리/server` 가 바뀌면 여기도 같이 바꿔야 한다.
   ✅ `web/tools/release-sync.mjs` 가 그 갈림을 검사한다(web-engineer 도구).

**아래는 이 폴더가 만들어질 때의 진단이다. 값은 그대로 유효하다.**

### ✅ 잰 것 — 이식성 자체는 이미 있다
```
`_WIN32` 분기 **17곳** (runtime 9 · server.cpp 3 · metrics 2 · nodes/persist/wire 1씩)
POSIX 전용 헤더가 `#ifdef` 밖으로 새는 것 → **0건**
MSVC 에 없는 이름(`ssize_t`·`strcasecmp`·`__attribute__`·`typeof`) → **0건**
시각 : `GetTickCount64` · `GetSystemTimeAsFileTime` 로 이미 갈려 있다
🔑 분기가 **소켓·시각 계층에 갇혀 있다** — 흩어져 있지 않다
```

### 🔴 실제로 잡은 결함 하나 — `Gate::IN` (고쳤다)
```
`<windows.h>` 는 `IN`·`OUT` 을 **빈 매크로**로 정의한다(SAL 주석용)
→ `enum Kind { IN, OUT };` 이 `enum Kind { , };` 로 펼쳐진다
```
**Windows 없이 재현했다** — 매크로를 흉내 내면 어디서든 난다:
```
c++ -fsyntax-only -DIN= -DOUT= -x c++ parking.h
→ parking.h:44: error: expected identifier      ← 고치기 전
→ (통과)                                        ← 고친 뒤
```
🔑 **`parking.h` 가 스스로 `#undef` 한다.** `server.cpp` 쪽에서 걷으면
**다른 포함 순서에서 다시 깨진다** — 기여자의 파일이 그렇다.
> **이름을 가진 헤더가 그 이름을 지킨다.**
⚠ **이 결함은 옛 단일 파일(`vs_win_server.cpp`)에는 없었다** — 조립 API(`parking.h`)가
새로 생기면서 들어왔다. **옛 포팅이 통과했다는 사실이 지금을 보증하지 않는다.**

### 🔴 같은 자리에서 하나 더 — `Measure::min` 은 **남의 `#define` 에 기대고 있었다** (2026-08-27)

```
parking.h  IN · OUT · near · far   → 스스로 `#undef` 한다
parking.h  Measure& min(long cm);  → 🔴 **`min` 은 그 목록에 없었다**
```
지금 안 터진 이유는 **다른 파일**에 있었다 — `server.cpp:62 #define NOMINMAX`.

**값으로 쟀다** (하니스에 `min`/`max` 매크로를 켜서 = `NOMINMAX` 가 없는 상황):
```
고치기 전 → 🔴 exit=1
   parking.h:170  error: too few arguments provided to function-like macro invocation
   parking.h:173  error: constructor for 'Measure' must explicitly initialize ... 'min'
고친 뒤   → ✅ exit=0
```
🔴 **`min` 은 함수형 매크로**라 `near`/`far`(빈 매크로)와 증상이 다르다 — 인자 개수 오류로 뜨고
   **생성자까지 번진다.** 그래서 오류 목록이 길어지고 **첫 원인이 안 보인다.**
🔑 `NOMINMAX` 를 지우지 않는다 — 그쪽은 `std::min`/`std::max` 를 지키고 여기는 **우리 이름**을 지킨다.
★ 판별자는 *"이 헤더가 스스로 서 있나"* 다. `parking.h` 는 **기여자가 include 하는 공개 API** 라
  `<windows.h>` 를 먼저 넣은 남의 번역 단위에서도 서야 한다.

### 🔴 남은 함정 (우리가 못 재는 것) — 직접 만들 때 쓰라
```
① ✅ **고쳤다 (2026-08-20).** 전에는 `server.cpp` 가 `#include "lot.cpp"` 하는
   **유니티 빌드**여서, VS 가 폴더의 `.cpp` 를 자동 추가하면 `lot.cpp` 가 두 번 컴파일되고
   `buildLot`·`onTick`·`onCmdResult` 가 **중복 정의**됐다 → `LNK2005`
   ✅ 훅 셋의 **선언을 `parking.h` 로** 올리고 `#include "lot.cpp"` 를 뺐다.
     `lot.cpp` 는 이제 **진짜 번역 단위**다 →
     🔑 **폴더의 `.cpp` 를 전부 컴파일하는 것이 이제 *정답* 이다**
   ⚠ macOS 에서 그 오류를 **재현하고 고쳤다**:
     `c++ -c lot.cpp && c++ -c server.cpp && c++ lot.o server.o`
     → 고치기 전 `duplicate symbol` 3건 · 고친 뒤 **링크 성공**
② 🔴 **`/utf-8` 을 반드시 준다.** 이 소스는 BOM 없는 UTF-8 이고 안내 문구가 한글이다.
   빼면 CP949 로 읽어 **문자열이 깨지는데 컴파일은 통과한다** —
   **빌드 로그로 못 잡고 브라우저 화면에서 처음 보인다**
③ 링크에 **`ws2_32.lib`**
④ `/std:c++14` 이상 (코드는 C++11 이다)
```
🔴 **빌드 오류가 나면 `파일:줄`과 오류 코드를 알려 달라.**
`LNK2005` 면 ①이고, `C####` 면 다른 것이다 — **한 글자로 갈린다.**

---

## 고칠 파일은 **`lot.cpp` 하나다**

```cpp
lot.spot("A1").sensor("P1","A1").actuator("P1","LD");   // 조립
lot.label("P1","LD","안내등");                           // 표시 이름
lot.control("P1","LD").toggle();                         // 화면 조작 UI
```
설명은 같은 폴더의 `GUIDE-sample.md` 에 있다. **이 파일은 따라 치는 것이고 그 문서는 읽는 것이다.**

### 다 만들면 운영으로 옮긴다
`lot.cpp` **하나만** `조별과제샘플/server/` 로 복사하면 된다 — **그것이 유일한 이음매다.**
```
sh net/sync_check.sh   → 두 트리 모두에서 lot.cpp 가 컴파일되는가
```
🔑 공개 API(`parking.h`)가 갈리면 **여기서 빨강**이 난다. 사람이 안 세도 된다.

---

## 포트 — 운영과 **일부러** 다르다

```
dev  : 웹 9900 · 아두이노 9991 · 카메라 5500     ← 기본값. 손잡이 없이 그냥 뜬다
운영 : 웹 9990 · 아두이노 8888 · 카메라 8911
```
🔴 **근거는 실패 방향의 비대칭이다:**
```
dev 값으로 운영에 쓰면  → **안 붙는다.** 즉시 드러난다
운영 값으로 dev 에 쓰면 → 🔴 **운영에 붙는다.** devid 가 겹치면 남의 노드를 쫓아내고,
                          **로컬에서는 멀쩡해 보인다**
```
> **틀렸을 때 조용한 쪽을 기본값으로 두지 마라.**

바꾸려면: `./srv --port-web=9901 --port-ardu=9992 --port-cam=5501`
⚠ 셋이 서로 겹치면 **어느 둘인지 이름으로 말하고 안 뜬다.**

### 아두이노도 같이 맞춰야 한다
`조별과제샘플/ardu/Config.h` 의 **`SERVER_IP` 와 `SERVER_PORT` 를 한 벌로** 본다 —
포트만 고치고 IP 를 안 고쳐서 헤매는 일이 실제로 있었다.

---

## 🔴 운영판에서 **뺀 것** (있었는데 없앤 것)

| 뺀 것 | 왜 |
|---|---|
| 자가검증 5파일 **2,300줄** | 배포 관문이지 디버그 도구가 아니다 |
| 소크 요약 (60초마다 **1,329자** + 종료 총평) | 운영 관측. **콘솔에서 못 읽고** 프레임 로그를 밀어낸다 |
| 기계용 `=== INSTANCE logfmt=…` 경계 줄 | monitor 가 한 파일에 쌓인 인스턴스를 자르려고 읽는 줄. **여기선 자를 것이 없다** |
| 계약값 덤프 2줄 (**289자·188자**) | arduino·monitor 가 계산에 쓴다. 개발자는 안 쓴다 |
| 로그 **파일** (`--log=` · `open_log`) | 출력은 stdout 하나다. 남기려면 `./srv > dev.log` |
| rid 커서 · 노드 대장 영속 | 🔴 `~/parking-logs` 를 **아예 안 건드린다.** 재기동하면 잊는다 — **개발엔 그게 낫다** |
| `exe_path` · `iso8601` · `open_log` · `default_log_path` · `lock_log_exclusive` | 위 것들에 **딸려 죽었다.** 🔑 컴파일러가 `-Wunused-function` 으로 알려 줬다 |

**10,158줄 → 7,681줄 (−24%)**

### ⚠ 그런데 `data_log.json` 은 **아직 쓴다** — 처음에 "영속 전부 뺐다"고 적었다가 고쳤다
```
실행 폴더에 `data_log.json` 이 생긴다 (주차 기록 · 명세 §9)
🔑 그건 **관측이 아니라 제품 기능**이다. `~/parking-logs`(운영 관측)와 다른 것이다
```
> **"전부"라고 쓰기 전에 세라.** 기동 로그가 `data_log.json 이 없어 빈 배열로 만들었다` 라고
> 말하고 있었는데 나는 "영속 전부 뺐다"고 적었다. **한 줄 위에 반증이 떠 있었다.**

## ⚠ **못 뺀 것** (빼려고 봤지만 남긴 것)

```
metrics.h · ledger.h  →  `nodes.h`·`uplink.h` 와 얽혀 있다
```
🔴 빼려면 **전선 코드(상행 파서)를 고쳐야 하고, 그러면 두 트리의 전선이 갈린다.**
**그게 가장 위험한 갈림이다.** → **호출만 끊어 출력을 없앴다.** 코드는 남고 콘솔은 조용하다.

## ⚠ 아직 **안 넣은 것** ("뺐다"가 아니다)

```
index.html — 🔴 web-engineer 가 넣는다. 이 폴더에서 그 파일만 web 소유다
             🔑 **갱신 주체와 소유자를 갈라 놓으면 반드시 낡는다**
             그때까지 `GET /` 는 404 다. 기동 로그가 그렇게 말해 준다
```

---

## 출력은 **stdout · `std::cout`** 이다 (사용자 확정)

파일로 안 쓴다. 개발자가 **눈으로 읽는다.**
그래서 운영판의 200자 넘는 줄들을 걷어냈다 — 지금 최장 줄은 **125자**(구분선)다.

---

## 🔑 세 트리의 차이 — **의도된 것과 낡은 것을 갈라 적는다**

```
server/       운영. **unity 빌드**(`#include "lot.cpp"`) · `selftest_*` **8개를 싣는다**
dev_server/   기여자 배포. **진짜 번역 단위**(VS 가 폴더의 .cpp 를 전부 컴파일한다)
VS_server/    Windows. dev 와 **같은 소스** + VS 프로젝트 파일
```
| 무엇 | 어디 | 왜 — **의도다** |
|---|---|---|
| `selftest_*` 8개 | **server 에만** | 🔑 기여자 배포에 시험을 안 싣는다. 배포 묶음이 작아야 하고, 기여자가 고칠 것도 아니다. ⚠ **낡아서 빠진 것이 아니다** |
| `EXAMPLES.cpp` · `GUIDE-*.md` · `README.md` | **dev·VS 에만** | 기여자가 읽는 것. 운영 트리에 둘 이유가 없다 |
| `index.html` | dev·VS 에만 | **web 소유**. 서버 소스가 아니다 |
| `config.h` 포트 3줄 | server ≠ dev·VS | 동시에 돌려야 한다(운영 8888/9990/8911 · dev 9900/9991/5500) |
| `entry.h` · `listen.h` · `runtime.h` · `serve.h` | server ≠ dev·VS | dev 는 **콘솔 전용**(파일 로그 없음 · 자가검증 없음 · 소크 요약 안 찍음 · 실패를 사람 말로) |
| `parking.h` 훅 선언 · `server.cpp` include | server ≠ dev·VS | `lot.cpp` 가 진짜 번역 단위라서 |
| ~~`entry.h` 의 `#include <windows.h>` · `SetConsoleOutputCP(65001)`~~ | ~~VS 에만~~ | 🔴 **정본에도 있다**(`entry.h:20~33`, `#ifdef _WIN32` 로 감싼 채) |
| ~~`server.cpp` 의 `#define _CRT_SECURE_NO_WARNINGS`~~ | ~~VS 에만~~ | 🔴 **정본에도 있다**(`server.cpp:33`) |
| `server.slnx` · `.vcxproj` · `.filters` | VS 에만 | VS 빌드 정의 |
| 🔵 `user8080.html` · `user8081.html` | **이 폴더에만**(샘플엔 없다) | **의도한 것이다** — 이용자 화면 둘(`web` 소유). 샘플은 그 화면이 생기기 전 판이다. 서버가 **포트마다 다른 문서**를 준다(`serve.h:153·161`) : `9900 → index.html` · `8080 → user8080.html`(주차위치 확인) · `8081 → user8081.html`(자리 선택). `F5` 인자의 `--web-root=$(ProjectDir).` 가 이 폴더를 뿌리로 잡아 **셋 다 나온다** |

> ### 🔴 **위 표에서 취소선 그은 둘은 2026-08-27 에 거짓이 됐다.**
> ### 정본이 그 코드를 흡수했다 — **`서머리/server` 와 이 폴더의 `.h`/`.cpp` 34개는 지금 바이트 동일이다.**

## ✅ 이 트리는 이제 **자동 검사 안이다** (2026-08-27)

```
python3 net/release_sync_check.py          검사만 (다르면 exit 1)
python3 net/release_sync_check.py --sync   정본 → 사본으로 맞춘다
```
```
① 내용        : 정본 34개 == 릴리즈/server == 릴리즈/VS_server/server · 사본끼리도 댄다
② 빌드 대상   : `.vcxproj` · `.filters` · 맥 빌드 줄이 **같은 `.cpp` 를 컴파일하나**
③ 문서 예제   : `EXAMPLES.cpp` 가 정본 `parking.h` 로 여전히 컴파일되나
```

### ⚠ 전에 안 넣었던 이유와, 그것이 왜 무너졌나

전에는 *"의도된 Windows 전용 차이가 넷 있어서 파일 대조를 걸면 매번 발화한다"* 였다.
🔑 **그 논리는 맞았다.** 다만 **전제가 사라졌다** — 그 넷 중 **코드 둘이 정본으로 올라갔고**,
남은 둘(`.vcxproj`·`.filters`)은 **정본에 없는 파일**이라 `.h`/`.cpp` 대조의 분모에 안 들어간다.

> ### ★ **"항상 발화하는 검사는 아무 말도 안 하는 검사다" 를 피하는 법은 검사를 빼는 것이 아니라**
> ### **분모를 정확히 고르는 것이었다.** 빼 두는 동안 `릴리즈/server` 가 실제로 셋 낡았다.

**대신 지금 걸려 있는 것 · 갱신하는 사람이 해야 할 것:**
```
✅ EXAMPLES.cpp 는 VS 빌드에 들어 있다(ClCompile) — **거기서 컴파일이 검증된다**
✅ 우리 쪽에서도 그 파일을 c++11/14/17 · -DIN= -DOUT= 로 문법 검사했다
🔴 소스를 갱신할 때는 **`dev_server/` 를 정본으로 삼고 내용을 적용해라**(파일 덮어쓰기 금지).
   위 넷은 이 트리 것이므로 **살려 둬라**
```

⚠ **우리는 Windows 가 없다.** MSVC 로 실제 빌드한 적이 없으므로 *"빌드된다"* 는 **미검증**이다 —
전처리기 수준(빈 매크로 `IN`/`OUT`)까지만 재현해서 확인했다.
