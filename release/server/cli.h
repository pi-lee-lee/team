...
// ═══════════════════════════════════════════════════════════════════
    // 로그 경로(로그 계약 §2.4) — 비워 두면 기본 경로를 쓴다.
    ...
    // ── 🔴 8081 자리 선택 서비스 **일시 중지** (사용자 지시 2026-08-27)
    //   ★ **지우는 것이 아니라 끄는 것**이다. 빼면 그대로 돌아온다.
    for (int i = 1; i < argc; i++) {
        if (std::string(argv[i]) == "--no-chooser") {
            g_chooser_on = false;
            std::cout << "*** --no-chooser — 8081 **자리 선택 서비스 중지**. "
                         "선택 화면이 붙어 있어도 **자동 배정**한다.\n"
                         "    (화면은 살아 있다 · 차단봉 열기와 안내등이 같은 초에 난다) ***\n";
        }
    }

    // ── 🔵 뿌리 둘 (REQ-0497) — `--web-root`(읽기·화면) · `--data-dir`(쓰기·상태)
    //   🔴 **cwd 기본값을 두지 마라.** 그러면 *"어디서 띄웠느냐"* 에 따라 화면이 갈리고,
    //     그 갈림은 **`ls` 도 `curl 200` 도 통과한다**(§"존재형은 유물을 통과시킨다").
    //   ⚠ 못 찾아도 **기동은 한다** — 화면은 관측이고 주차 제어는 기능이다.
    //     **관측 하나가 기능을 죽이면 안 된다**(루트 승인 · 내가 낸 "기동 거부" 안은 철회했다).
    for (int i = 1; i < argc; i++) {
        std::string a(argv[i]);
        const char* keys[2] = { "--web-root=", "--data-dir=" };
        std::string* dst[2] = { &g_docroot, &g_dataroot };
        for (int k = 0; k < 2; k++) {
            const size_t kl = strlen(keys[k]);
            if (a.size() <= kl || a.compare(0, kl, keys[k]) != 0) continue;
            std::string v = a.substr(kl);
            // 🔴 **끝의 구분자를 뗀다 — `/` 만이 아니라 `\` 도**(web 이 짚었다 · 2026-08-27).
            //   ★ MSVC 관례로 **`$(ProjectDir)` 는 끝에 `\` 가 붙는다.** 그대로 두면
            //     `doc_path()` 가 `...server\` + `/` + `index.html` = **`...server\/index.html`** 을 만든다.
            //     🔵 윈도우가 대개 받아 주지만 **로그와 비교에서 어긋난다** —
            //       기동 줄의 `정적 뿌리 <경로>` 가 남과 다른 모양으로 찍혀 대조가 깨진다.
            //   ⚠ POSIX 에서 `\` 는 **정상 파일명 문자**다. 그런데 이 값은 **디렉터리 경로**라
            //     끝에 `\` 로 끝나는 디렉터리를 쓸 일이 사실상 없다 — 떼는 쪽이 안전하다.
            while (v.size() > 1 && (v[v.size()-1] == '/' || v[v.size()-1] == '\\')) v.erase(v.size()-1);
            *dst[k] = v;
        }
    }

    // ── 🔴 포트를 **하나씩** 준다 (사용자 확정 · `--port-offset` 은 없다)
