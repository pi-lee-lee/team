...
struct ModuleMeasure {
    ...
    long value;   // 장치가 준 정수
    ModuleMeasure() : has(false), value(0) {}
    ModuleMeasure(bool h, long v) : has(h), value(v) {}
};
...
struct SpotBehavior {
    ...
    //
    // ═══ ⚠⚠ **자리 *활성* 판정은 AND 다 — 축이 다르다. 접지 마라** ═══
    //   ```
    //   자리 활성 : 선언된 센서가 **전부** 값을 아는가        → 🔴 **AND**   (여기가 아니다)
    //   점유 판정 : **활성인 동안** 아는 센서가 뭐라고 하나   → **OR**      (이 함수다)
    //   ```
    //   🔑 **순서가 있다: 먼저 활성인지 보고, 활성일 때만 점유를 말한다.**
    //     비활성이면 점유는 `unknown` 이고 이 함수의 답은 쓰이지 않는다.
    //   ★ 그래서 위의 *"AND 로 하면 영영 비었다로 보인다"* 는 **여전히 참이다** —
    //     그 문장은 **점유 축**의 이야기이고, 활성 축의 AND 와 모순되지 않는다.
    //     (센서가 죽으면 이제 **자리가 비활성**이 되므로 "영영 비었다"로 보일 일 자체가 없다)
    //
