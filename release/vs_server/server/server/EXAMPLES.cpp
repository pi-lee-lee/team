...
// ── ③-b 🔴 **여러 보드로 늘린다 — 확장은 세로가 아니라 가로다** ────────────
//   한 보드에 센서를 많이 붙이면 **그 보드의 회선**이 먼저 막힌다
//   (센서 6개부터 하행 묶음이 4 → 3 으로 준다 · `docs/net/SPEC-sensor-value.md` §9).
//   🔑 **보드를 늘리면 회선이 장치마다 따로다.** 조별과제 구조와 그대로 맞는다.
//   ⚠ 서버가 받는 보드 수 상한은 **8** 이다(`MAX_ARD_NODES`). 넘으면 **새 연결을 거절한다** —
//     살아 있는 노드를 쫓아내지 않는다.
void example_many_boards(ParkingLot& lot) {
    // 내 보드
    lot.spot("A1").at(0, 0).parking().label("1번").module("P1", "A1");
    // 🔓 남의 보드 — **devid 만 다르다.** 그 사람이 자기 client.ino 에 그 id 를 적는다
    lot.spot("A6").at(1, 0).parking().label("6번").module("KIM01", "C1");
    lot.spot("A7").at(1, 1).parking().label("7번").module("LEE02", "D1");
    // 🔑 명령도 같다 — `srv.send("KIM01", "LD", 1)` 로 **남의 보드**에 지시한다(example_send 참고)
}
...
void example_module_value(ParkingServer& srv, const std::string& spot,
...
// ── ⑨ 카메라 pull — **촬영을 요청하고 요청번호로 번호판을 회수한다** ────────
//   계약 정본 : docs/net/SPEC-camera-pull.md
//
// 🔴 **비동기다.** `cameraShoot()` 은 요청번호만 주고 바로 돌아온다 —
//   번호판은 폰이 찍어서 보내 줄 때까지 **없다.** 그 동안 `cameraPlate()` 는 `"-1"` 이다.
//
// 🔑 그래서 **한 박자 안에서 요청하고 바로 읽으면 영원히 "-1" 을 본다.**
//   요청번호를 **들고 있다가** 다음 박자들에서 다시 물어라.
void example_camera(ParkingServer& srv) {
    static long long shot = -1;             // 🔓 들고 있어야 한다 — 그래서 static 이다

    if (shot < 0) {
        shot = srv.cameraShoot();           // 🔴 -1 이면 폰이 안 붙었거나 오늘 seq 를 다 썼다
        if (shot < 0) return;
        srv.log("촬영 요청 " + std::to_string(shot));
        return;
    }

    // 🔑 **`cameraPlate()` 하나만 봐도 틀리지 않는다** — "-1" 이면 번호판이 없는 것이 참이다.
    //   다만 *"기다리면 오나"* 를 알려면 상태를 봐야 한다.
    const int st = srv.cameraState(shot);
    if (st == CAM_PENDING) return;          // 아직 온다. 기다린다

    if (st == CAM_READY) srv.log("번호판 " + srv.cameraPlate(shot)
                                 + " (" + std::to_string(srv.cameraAge(shot)) + "ms)");
    else                 srv.log("촬영 실패 또는 만료 — 상태 " + std::to_string(st));

    // ⚠ 서버는 **시한을 안 정한다.** "너무 오래 걸렸다" 는 네가 정한다:
    // if (st == CAM_PENDING && srv.cameraAge(shot) > 10000) { shot = -1; }   // 10초면 포기

    shot = -1;                              // 다음 요청을 낼 수 있게 비운다
}
