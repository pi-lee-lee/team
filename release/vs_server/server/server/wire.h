// wire.h — 송신 helper · WebSocket 프레임 (§5.2). `struct Server` 의 몸통 조각 — server.cpp:181
// ⚠ 단독 컴파일 불가 · include 자리가 곧 문법이다 · 📖 server.cpp "목차"
    ...
    // 🔑 `timeout_ms` 는 **이 루프 전체**의 마감이다. 소켓 옵션(`SO_SNDTIMEO`)은 한 호출만 막으므로
    //   **둘 다** 같은 값이어야 한 호출이 창을 먹는 경로가 닫힌다.
    bool send_raw(sock_t fd, const char* p, size_t n, const char* who,
                  int timeout_ms = SEND_TIMEOUT_MS) {
        size_t off = 0;
    ...
    // 🔑 `send_ard()` 가 여기 있었다. **호출자가 0 이 되어 지웠다** —
    //   하행이 노드별 소켓으로 갈리면서 모든 경로가 `send_node()` 를 탄다.
    //   ⚠ 호출자 없는 코드는 **있는 것처럼 읽힌다.** 남기지 않는다.
    //   ★ 그 안에 있던 **착지 위상 계측은 아래로 옮겼다** — 지우면 `W` 를 정할 입력이 사라진다.
    void send_node(Node& node, const std::string& line) {
        if (node.fd == BAD_SOCK) return;
        if (!send_raw(node.fd, line.data(), line.size(), node.devid.c_str())) {
            node_send_failed(node, "송신 실패");
            return;
        }
        ...
        //
        // 🔴 **왜 로그 본문에 넣나**: 줄 앞머리 시각이 **1초 해상도**라
        // 조용한 창(약 1.0초)과 장치 활성 구간(약 113ms)을 **구별할 수 없다.**
        // → 이것 없이 `W`(송신 허용 창)를 정하면 **추측이다.**
        //
        // ⚠ **`S` 를 한 번도 못 받은 상태를 따로 표시한다.** `seen` 이 false 면
        // `last_ms` 는 **의미 없는 0** 이고, 그때 차이를 찍으면 **거대한 가짜 값**이 된다.
        //
        // 🔴 **노드별로 잰다.** 전역 `ard_last_ms` 로 재면 P2 로 나간 줄의 위상을
        //   **P1 의 주기**에 대고 재게 된다 — 값이 그럴듯해서 아무도 안 알아챈다.
        // 🔴 **`devid` 는 항상 둘째 토큰이다** — `→ARD <devid> <프레임> (위상)`.
        //   관측 도구가 **위치로** 장치를 가른다. 위상 괄호 안에 넣으면 자리가 프레임마다 달라진다.
        {
            char ph[64];
            if (!node.seen)
                snprintf(ph, sizeof(ph), " (S미수신)");
            else
                snprintf(ph, sizeof(ph), " (S+%lldms)", (long long)(now_ms() - node.last_ms));
            logf("→ARD", node.devid + " "
                         + (line.empty() ? std::string() : line.substr(0, line.size() - 1)) + ph);
        }
    }
    ...
    void clear_downq_for(const std::string& devid, const char* why) {
        std::vector<DownQ> tmp;
        for (size_t i = 0; i < downq.size(); ) {
            if (downq[i].devid == devid) {
                downq_bytes -= (long long)downq[i].line.size();
                tmp.push_back(downq[i]);
                downq.erase(downq.begin() + i);
            } else i++;
        }
        if (tmp.empty()) return;
        char b[192];
        snprintf(b, sizeof(b), "하행 큐 비움(%s/%s) — %zu줄. **전선에 나가지 않았다**",
                 devid.c_str(), why, tmp.size());
        logf("✂", b);
        q_dropped_link += (long long)tmp.size();
    ...
    void clear_downq(const char* why) {
        clear_downq_for(park.devid, why);
    }
    ...
    bool enqueue_down(Pending& p, const std::string& line, bool announce, bool force,
        ...
        //  소켓 없는 경로에서 큐가 쌓이는 것을 보고 찾았다.)
        Node* target = node_by_devid(p.devid);
        if (!target || target->fd == BAD_SOCK) {
        ...
        if (!force && !important) {
            ...
            for (size_t k = 0; k < downq.size(); k++)
                if (downq[k].devid == p.devid && !downq[k].important) user_n++;
        ...
        DownQ q;
        q.devid = p.devid; q.line = line; q.ws_fd = p.ws_fd; q.brid = p.browser_rid; q.slot = p.slot;
    ...
    void flush_downq(Node& node, const char* why, bool ignore_window, int max_n = DOWN_BATCH_MAX_N) {
        if (downq.empty() || node.fd == BAD_SOCK) return;
        long long t = now_ms();
        if (!ignore_window && node.seen) {
            // 창 안으로 얼마나 들어왔나. 지났으면 **다음 `S` 를 기다린다** —
            // 늦게 쏘면 장치의 송신 구간을 침범하고, 그게 §8.15 의 UART 혼재다.
            long long into = t - node.last_ms;
            if (into >= w_srv()) {
                win_skips++;
                char b[160];
        ...
        for (int pass = 0; pass < 2; pass++) {
            ...
            for (size_t k = 0; k < downq.size(); ) {
                ...
                if (q.devid != node.devid) { k++; continue; }
                if (q.important != want) { k++; continue; }
                // 🔴🔴 **창 원자성** — 배치는 **통째로 들어가거나 통째로 미뤄진다.**
                //
                //   ⚠ 큐에 있는 것을 순서대로 담지 마라. 그러면 **내 셋을 넣기 전에 남의 것이
                ...
                if (q.batch_id != 0 && !skipped_batch.count(q.batch_id)) {
                    ...
                    for (size_t j = 0; j < downq.size(); j++)
                        if (downq[j].devid == node.devid && downq[j].batch_id == q.batch_id) {
                            need_b += downq[j].line.size(); need_n++;
                        }
        ...
        if (!send_raw(node.fd, payload.data(), payload.size(), node.devid.c_str())) {
            ...
            node_send_failed(node, "하행 전송 실패");
            return;
        }
        batch_count++;
        ...
        for (size_t i = 0; i < batch.size(); i++) {
            ...
            // 🔴 **`devid` 는 항상 둘째 토큰이다** — `→ARD <devid> <프레임> (위상)`.
            //   관측 도구가 **위치로** 장치를 가른다. 위상 괄호 안에 넣으면 프레임마다 자리가 달라진다.
            char ph[64];
            if (!node.seen) snprintf(ph, sizeof(ph), " (S미수신)");
            else            snprintf(ph, sizeof(ph), " (S+%lldms)", (long long)(t - node.last_ms));
            std::string l = batch[i].line;
            if (!l.empty() && l[l.size()-1] == '\n') l.erase(l.size()-1);
            logf("→ARD", node.devid + " " + l + ph);
        }
        {
            char b[192];
            size_t remain_n = 0;
            for (size_t i = 0; i < downq.size(); i++) if (downq[i].devid == node.devid) remain_n++;
            snprintf(b, sizeof(b), "배치 %zu줄 · %zuB · device=%s (%s) · cap %dB · 해당 node 남은 큐 %zu줄",
                     batch.size(), payload.size(), node.devid.c_str(), why, DOWN_BATCH_CAP_B, remain_n);
            logf("⇉", b);
        }
        // 미룬 것을 **조용히 미루지 않는다** — 안 적으면 지연이 아무 데도 안 보인다(원장 §5.2).
        size_t remain_n = 0; long long remain_b = 0;
        for (size_t i = 0; i < downq.size(); i++) if (downq[i].devid == node.devid) {
            remain_n++; remain_b += (long long)downq[i].line.size();
        }
        if (remain_n) {
            q_deferred++;
            char b[160];
    ...
    void ws_send(sock_t fd, const std::string& payload) {
        ...
        if (!send_raw(fd, f.data(), f.size(), "WS 클라이언트", SEND_TIMEOUT_WS_MS)) dead_conns.push_back(fd);
    }
    ...
    void reap_dead() {
        for (size_t i = 0; i < dead_conns.size(); i++) {
            ...
            if (conns.count(fd)) { closesock(fd); conns.erase(fd); conn_doc_.erase(fd); choosers_.erase(fd); logf("-WS", "전송 실패로 연결 종료"); }
