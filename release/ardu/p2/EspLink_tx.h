#pragma once
// ═════════════════════════════════════════════════════════════════════════
// EspLink_tx.h — 🔒 **링크 계층**: `AT+CIPSEND` 프롬프트 대기 · `espWrite` (SEND OK 축의 본체)
// ═════════════════════════════════════════════════════════════════════════
//
// 🔴🔴 **이 파일들이 하드웨어 리셋을 불필요하게 만든 물건이다**
...
//   ⚠ **이건 사용자의 설계 확정이다.** 📖 원문: docs/arduino/LEDGER.md
//
...
// ─────────────────────────────────────────────────────────────────────────
// ── 프롬프트 대기 결과는 **셋이 아니라 넷**이다 ──
// 🔴 **`bool` 로 두지 마라**(`>` 를 봤는가 아닌가). 그러면 **ESP 가 "지금 바쁘다"고 명시적으로
// 답한 것**과 **아무 답도 없는 것**이 같은 값(false)으로 뭉개졌고, 둘 다 똑같이 실패로 세어
// 3번이면 멀쩡한 소켓을 끊었다. 링크는 문제가 없는데 우리가 끊는 것 — **자해**였다.
// (`busy s...` 거부가 `send_fail` 로 계상되는 것을 AT 로그로 확인)
...
static bool waitForPromptIsReject(char c, uint8_t* mBusy, uint8_t* mErr, uint8_t* out) {
  ...
  // ⚠ 대소문자를 가리지 않는다 — `ERROR` 와 `Error` 가 **둘 다** 온다.
...
// ─────────────────────────────────────────────────────────────────────────
// 🔴 몸통을 `espWrite()` 로 뺐다. **한 거래에 여러 줄**을 담기 위해서다.
//
...
static bool espWrite(const char* line, uint16_t len) {
  ...
  if (awaitingSendOk) {
    ...
    lastSendOkMs = OKPREV_TO;                                    // REQ-0501 — 시한 포기는 값이다: okprev=TO
  ...
  inSend = true;
  // ★ **지금 보내는 배치 길이를 남긴다.** 수신 계수(`penddrop`/`okstream`)가
  ...
  if (ok) {
    // 🔴 **쓰면서 링버퍼를 비운다.**
    //
    // 🔴 `len` 바이트를 **통째로** 쓰고 그 사이 아무것도 안 꺼내면 안 된다.
    ...
    //
    // ★ 지문: `[AT]` 줄 길이가 **63 에 20건 몰려 있다.** `63 + LF = 64B` —
    ...
    //     (회귀 시험이 이 결함을 잡는다 — 코드보다 시험이 먼저 맞은 자리다)
    ...
    // (더미 뒤에 `Error`/`busy s...` 가 붙고 `SEND OK` 가 오지 않았다.)
    //
    // ❌ **틀린 설명 주의**: *"`#####` 가 **에코로 되돌아온 것**이 더미가
    ...
    //   ⚠ 그리고 `SEND OK` 는 **6초 뒤에 올 수도 있다.** 안 왔다고 단정하지 마라.
    ...
    // 파싱될 가능성이 없다.
    //
    // ✏️ **틀린 설명 주의** — *"체크섬 이전에 **타입**에서 걸린다"* 는 말이 있었는데
...
#if DEBUG
  // REQ-0501 — 내용이 안 바뀐 S 는 사건 창 밖에선 찍지 않는다. 실패·D·A 는 항상 찍는다
  if (!ok || line[0] != 'S' || echoOpen()) {
    Serial.print(ok ? F("[TX] ") : F("[TX-DROP] "));
    Serial.println(line);
  }
#endif
  // ★ 이 결과가 유일한 연속 실패 신호다. 여기서만 센다.
  //   goOffline() 이 netOnline 을 내릴 수 있으므로 inSend 를 내린 **뒤**에 부른다.
  //
  // ★ **`busy` 는 세지 않는다.** 연속 실패 카운터의 뜻은 "링크가 죽었을 것이다"인데,
