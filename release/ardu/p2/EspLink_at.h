#pragma once
// EspLink_at.h — AT 응답 어휘 해석 · handleLine — **링크 계층**
...
// TCP 접속 성공 줄인가? (판정을 넓히되 옛 버그는 되살리지 않는다)
//
...
// ─────────────────────────────────────────────────────────────────────────
// ⚠⚠ 구형 ai-thinker 펌웨어(AT v0.018 / SDK 0.9.2)의 **어휘가 다르다.**
...
//
// 앞의 둘을 못 잡으면 낡은 소켓 사다리와 캐시 비우기가 **둘 다 죽는다.**
...
static void handleFrameLine(char* cand) {
  ...
  //   🔴 `Q` 추가 — 서버가 "등록을 다시 보내라"고 묻는 프레임(명세 §5 ③).
  ...
  if (!checksumOk(cand, len)) {
    // ★ **계수는 `DEBUG` 밖.** 원문 출력만 안이다(원장 §8.4-2 의 경계:
    //   정수 계수는 밖 · 장문 진단은 안). 이 칸이 없으면 창 B 에서 손으로 못 찾고,
    //   그러면 "무손실"이 서버 계수만으로 선언된다.
    if (cksumNg < 65535) cksumNg++;
    echoEventAt = millis();                                       // REQ-0501 — 체크섬 실패는 사건: 60초 동안 정상 줄도 찍는다
#if DEBUG
  ...
  if (cand[0] == 'G') {
    ...
    uint16_t gidx = 0;
    uint32_t garg = 0;   // 🔴 **인자는 32비트** — LCD 7자리 같은 값이 온다
    ...
    // 🔴 **하드코딩 분기를 없앴다.** 어떤 모듈이 명령을 받을 수 있는지는
...
// REQ-0501 — 온라인 중 **성공 왕복 셋**(CIPSEND 에코 · 페이로드 에코 · SEND OK)은 변화 프레임/사건 창일 때만 찍는다.
//   그 밖은 전부 그대로다: AT+RST · ERROR · busy · ALREADY CONNECTED · Unlink · ready · CLOSED · CONNECT · +IPD ·
//   link is not valid · SEND FAIL (monitor 의 at_corrupt.py 가 이 낱말들로 UART 손상을 잡는다). 오프라인이면 게이트 자체가 꺼진다.
static bool atRoutineLine(const char* s) {
  if (!netOnline) return false;
  if (strcmp(s, "SEND OK") == 0) return true;
  if (strncmp(s, "AT+CIPSEND=", 11) == 0) return true;
  if (s[0] == ' ' && s[1] == 'S' && s[2] == ',') return true;   // 우리가 보낸 S 의 에코
  return false;
}
static void handleLine(char* s) {
#if DEBUG
  // ★ 받은 줄을 찍는다 — 단 온라인 중 성공 왕복 셋은 변화 프레임/사건 창일 때만(REQ-0501). 실패·리셋·busy 는 신호라 전부 찍는다.
  dbgLineCnt++;
  if (echoOpen() || !atRoutineLine(s)) {
    Serial.print(F("[AT] "));
    dbgLine(s, (uint8_t)strlen(s));
  }
#endif
  if (awaitingSendOk && strcmp(s, "SEND OK") == 0)              // REQ-0501 — CIPSEND→SEND OK 지연을 장치가 직접 잰다
    lastSendOkMs = (uint16_t)(millis() - sendOkWaitFrom);
  ...
  // 이제 AT 쪽을 놓친다. 그러나 링크 이벤트를 놓치는 것은 전송 실패 감지가 받아 주는 반면,
  ...
  // ═══════════════════════════════════════════════════════════════════════
  {
    ...
    if (ipd) {
      ...
      //     재려던 사건을 측정 지점이 밀어낸다. 그 결과 `oow` 가 0 으로 나온다 —
      //     **위반이 없어서가 아니라 못 재서다.**
      //   ✅ **줄의 첫 바이트를 받은 순간 확정된 오프셋**을 쓴다(`feedRxChar` 참조).
  ...
  //
  //     🔴 **버리기 전에 읽는다.** 곧장 return 하면 `WIFI GOT IP`(성공)와
  //     `WIFI DISCONNECT`(끊김)까지 같이 버려져 **"와이파이가 붙었는가"를 알 수단이 없어진다.**
  //     ⚠ 그렇다고 return 을 지우지 마라 — `WIFI CONNECTED` 가 isConnectLine 까지 흘러가
  //     오인 버그가 되살아난다. **return 은 두고 그 앞에서 상태만 걷어 간다.**
...
#endif
      // 여기서 곧장 CWJAP 로 되돌아가던 것이 **같은 자극의 반복**이었다.
  ...
  if (!netOnline && netLastSent < NET_STEP_COUNT) {
...
#endif
        // ★ 여기가 사다리의 주 입구다.
        //   ⚠ **곧장 CIFSR→CWJAP 로 돌아가지 마라** — 같은 자극의 반복이라 안 끝난다.
    ...
    if (netLastSent == NET_CIFSR) {
      if (strncmp(s, "AT+", 3) != 0 && hasUsableIp(s)) {   // 명령 에코는 제외
        ...
        if (!assocAt) assocAt = millis();   // 결합 유지시간 계측 시작
#if DEBUG
      ...
      // ★ **ESP 가 상태를 통째로 잃은 지문.**  (진단 전용 · 제어에 쓰지 않는다)
    ...
    // `no ip` = IP 없이 CIPSTART 를 쏜 것이다. **재시도가 아니라 CWJAP 로 되돌아간다**.
    if (strstr(s, "no ip")) {
      netHasIp = false;
#if DEBUG
  ...
  if (isAlreadyConnectLine(s)) {
    // 이 응답의 의미가 **상황에 따라 정반대**다. 둘을 갈라야 한다.
    if (staleSocket) {
    ...
    // **캐시를 비우지 않는다** — 새 연결이 아니므로.
    netOnline = true; echoEventAt = millis();                    // REQ-0501 — 온라인 복귀는 사건
    onlineSince = millis();
    if (hwRstPending) { if (hwRstOk < 65535) hwRstOk++; hwRstPending = false; }  // 4단이 들었다            // 사다리 복귀 판정의 기준 시각
  ...
  if (isClosedLine(s)) {
    netOnline = false;
    sendFailStreak = 0;
    // 🔴 **이 호출은 세 곳에 있다. 세다 틀리기 쉬워 여기 적어 둔다**:
    ...
    // 🔴 **여기서 곧장 CIPSTART 로 가지 마라.** 소켓이 죽은 이유가 **와이파이가 끊긴 것**일 수
    //   있다(공유기 재부팅 — 상시가동에서 가장 흔하다). 이 펌웨어는 `WIFI DISCONNECT` 를
    //   안 내므로 `netHasIp` 가 참으로 남고, 그러면 문지기를 통과해 **IP 없이 CIPSTART 가 나간다.**
  ...
  // 판정해도 "관계없는 AT 실패를 연결 문제로 오인" 이 구조적으로 생기지 않는다.
  //
  ...
  //   ⚠ 실제 문구는 아직 추정이다. `[AT]` 전체 로깅이 켜진 빌드에서 그 순간의
...
#endif
      // ★ 여기도 CIPSTART 로 바로 가면 무한 루프에 빠진다 — 같은 복구 경로를 탄다.
      startSocketRecovery();
      return;
    }
...
static void drainPending(void) {
  ...
  // ★ **도착 오프셋을 같이 넘긴다.** 이 경로가 정확히 "송신 중 도착"이라
