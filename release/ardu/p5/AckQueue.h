...
//   서버가 같은 `rid` 를 3번 재전송했고 **3번 다 같은 닫힌 게이트를 만났다.**
//
...
//   ⚠ RAM 이 빠듯하다(최저 여유 782B). 프레임을 통째로 담으면 한 칸 64바이트라 32배다.
//
// 멱등 캐시와 겹치지 않는다 — **직교한다:**
...
//   **사용자는 20~30번 연타한다.** `ackdrop +31`(서로 다른 rid 31개 — 아래 중복 방지가 있으므로).
 ...
 public:
  ...
  void dropUnbuildable() {
    while (qCount_ > 0 && find(q_[qHead_]) < 0) {
      ...
      qCount_--;
      if (stale_ < 65535) stale_++;   // ★ 큐 넘침(drops)과 **다른 사건**이다
#if DEBUG
