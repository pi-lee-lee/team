...
// Servo — **Timer2 하드웨어 PWM (OC2B = D3)**. 라이브러리를 안 쓴다.
//
// 🔴 왜 바꿨나 : `Servo` 는 **Timer1 ISR** 로 펄스를 끝낸다. 그런데 `SoftSerialBig` 은 한 바이트를
//   보내는 동안 **약 1.04ms 인터럽트를 막는다**(9600bps 비트뱅잉). 그 사이 펄스 종료 ISR 이 밀리면
//   1,472µs 로 낼 펄스가 2,500µs 가 되어 **차단봉이 90° 에서 180° 로 튄다.**
//   🔑 하드웨어 PWM 은 **ISR 이 없다.** 비교기가 핀을 직접 내리므로 `cli()` 와 무관하다 — 그 축이 사라진다.
//
// ⚠ **핀을 옮기지 마라.** D3 는 `OC2B` 라서 이 방식이 되는 것이다. 다른 핀이면 못 쓴다.
// ⚠ 그리고 이 스케치에서 **`tone()`·`analogWrite(3)`·`analogWrite(11)`** 을 쓰지 마라 —
//   셋 다 Timer2 를 가져가서 서보 펄스를 죽인다. (지금 0곳 · `grep` 으로 확인했다)
#define SERVO_EXIT 3                            // 🔴 D3 = OC2B. 옮기면 이 방식이 안 된다
const unsigned long SERVO_MOVE_TIME_MS=500UL;    // 목표 각도까지 이동하는 데 허용할 시간
bool exitDetachPending = false;
unsigned long exitDetachAt = 0;
static bool bootGatePending = false;             // 부팅 초기 자세는 예약만 한다 — 실행은 loop 에서 링크가 붙은 뒤(아래 serviceBootGate)
static const unsigned long BOOT_GATE_MAX_WAIT_MS = 30000UL;   // 링크가 안 붙어도 이 시간이 지나면 닫는다(정전 뒤 열린 채 방치 방지)

// 한 tick = 64µs (16MHz / 1024) · 주기 = 256 tick = 16.4ms · **펄스폭 = (OCR2B+1) × 64µs**
//   ⚠ 분해능이 64µs(약 6°)다. 90°/180° 두 자세에는 충분하지만 **미세 각도는 못 낸다.**
// 🔴 출구 서보는 **설치 방향이 입구(p3)와 반대**다 — 같은 펄스가 반대 각도를 만든다(사용자 실측).
//   그래서 p3 와 값을 맞바꿨다. 세 호출(열기·닫기·부팅 자세)이 이 두 상수를 쓰므로 여기서만 뒤집는다.
#define GATE_OPEN_TICKS   36                     // 37 × 64 = 2,368µs — 출구에서는 이것이 **열림**
#define GATE_CLOSE_TICKS  22                     // 23 × 64 = 1,472µs — 출구에서는 이것이 **닫힘**(부팅 자세)
static bool gateAttached = false;

static void gateServoAttach(uint8_t ticks) {
  OCR2B = ticks;                                 // 이중 버퍼 — 다음 주기 경계에서 반영된다
  if (gateAttached) return;
  digitalWrite(SERVO_EXIT, LOW);
  pinMode(SERVO_EXIT, OUTPUT);
  TCCR2B = 0;                                    // 🔴 멈춘 채로 설정한다 — 도는 중에 모드를 바꾸면 펄스가 깨진다
  TCNT2  = 0;
  OCR2A  = 255;                                  // TOP (Fast PWM 모드 7)
  TCCR2A = _BV(COM2B1) | _BV(WGM21) | _BV(WGM20);            // Fast PWM · TOP=OCR2A · OC2B 비반전
  TCCR2B = _BV(WGM22)  | _BV(CS22) | _BV(CS21) | _BV(CS20);  // 클럭 /1024 → 시작
  gateAttached = true;
}

static void gateServoDetach(void) {
  TCCR2A = 0; TCCR2B = 0;                        // OC2B 분리 + 클럭 정지 → 펄스가 없다
  digitalWrite(SERVO_EXIT, LOW);                // 🔑 놓을 때 핀을 LOW 로 — 떠 있으면 서보가 떤다
  gateAttached = false;
}
// 🔴 **언제 열고 닫을지는 장치가 안 정한다 — 서버가 정한다.**
//   장치에는 입·출차 시한이 없다. 받은 ED/XD 를 그대로 수행할 뿐이다.
...
// Exit Gate
// 1 = OPEN 0 = CLOSE   (서버는 toggle 이라 0/1 을 보낸다)
// SERVER 가 판단해서 명령을 보냄
// 아두이노는 이유를 판단하지 않고 그대로 실행
 
static bool cmdExitGate(uint32_t arg){ 
  bootGatePending = false;              // 명령이 부팅 초기 자세를 이긴다 — 예약이 남아 있으면 방금 받은 열기를 덮어쓴다
  switch(arg){
    case 1:
      gateServoAttach(GATE_OPEN_TICKS);
       exitDetachPending = true;
       exitDetachAt = millis();
#if DEBUG
      Serial.println(F("[XD] EXIT GATE OPEN"));
#endif
      return true;
    case 0:
       exitDetachPending = false;
      gateServoAttach(GATE_CLOSE_TICKS);   // 0 = CLOSE (서버 toggle 의 끔)
      exitDetachPending = true;
      exitDetachAt = millis();
#if DEBUG
      Serial.println(F("[XD] EXIT GATE CLOSE"));
#endif
      return true;
    default:
#if DEBUG
      Serial.print(F("[XD] 거절 — 잘못된 arg: "));
      Serial.println(arg);
#endif 
      return false;
  }
...
static bool commandEchoValue(const char* name, uint32_t arg) {
  if (strcmp(name, "ED") == 0 || strcmp(name, "XD") == 0) return arg == 1;
  return arg != 0;
}
#define COMMAND_ECHO_VALUE(name, arg) commandEchoValue((name), (arg))
...
#endif
  // 🔴 부팅 초기 자세(닫힘)는 여기서 **움직이지 않는다** — 예약만 한다.
  //   setup 직후 500ms 서보 구동이 첫 AT+RST(+0.5s)·ESP 결속 스파이크와 겹친다. 서보가 스토퍼에 눌려
  //   스톨이면 그 순간 5V 가 처져 결속 실패/브라운아웃이 된다. 링크가 붙은 뒤(또는 30s 뒤) loop 에서 움직인다.
  bootGatePending = true;
...
// 부팅 초기 자세 — 링크가 붙은 뒤(netOnline) 또는 상한 뒤에 한 번만. 그 뒤는 명령 경로와 같은 detach 규율.
static void serviceBootGate() {
  if (!bootGatePending) return;
  if (!netOnline && millis() < BOOT_GATE_MAX_WAIT_MS) return;
  bootGatePending = false;
  gateServoAttach(GATE_CLOSE_TICKS);
  exitDetachPending = true;
  exitDetachAt      = millis();
#if DEBUG
  Serial.println(F("[XD] BOOT GATE CLOSE (after link)"));
#endif
}
