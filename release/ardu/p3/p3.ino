...
#define SERVO_ENTRY 3                            // 🔴 D3 = OC2B. 옮기면 이 방식이 안 된다
const unsigned long SERVO_MOVE_TIME_MS=500UL;    // 목표 각도까지 이동하는 데 허용할 시간
bool enterDetachPending = false;
...
#define GATE_OPEN_TICKS   22                     // 23 × 64 = 1,472µs (Servo 라이브러리의 90° 와 같은 값)
#define GATE_CLOSE_TICKS  36                     // 37 × 64 = 2,368µs (180° ≈ 2,400µs)
...
static void gateServoAttach(uint8_t ticks) {
  ...
  digitalWrite(SERVO_ENTRY, LOW);
  pinMode(SERVO_ENTRY, OUTPUT);
...
static void gateServoDetach(void) {
  ...
  digitalWrite(SERVO_ENTRY, LOW);                // 🔑 놓을 때 핀을 LOW 로 — 떠 있으면 서보가 떤다
  gateAttached = false;
}
...
static void serviceBootGate() {
  ...
  enterDetachAt      = millis();
#if DEBUG
  Serial.println(F("[ED] BOOT GATE CLOSE (after link)"));
#endif
}
