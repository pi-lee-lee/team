// metrics.h — 소크 관측(REQ-0065) · 복구 계측(REQ-0072) · 지표 문장. `struct Server` 의 몸통 조각.
    ...
    Server() : grid_rows(5), grid_cols(5),   // 선언 순서(-Wreorder)
           ...
           q_nodev(0), q_dup(0),
               ...
               ack_unknown_rid(0), ack_slot_mismatch(0), mod_name_conflict(0), mod_dup_name(0), mod_unbound(0), asm_warn_(0), devid_example_(0), sensor_split_now(0), not_reservable_n(0),
