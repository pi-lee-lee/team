...
// ⑤ 센서 값이 올 때마다 불린다 — 🔴 **점유가 안 바뀌어도 온다**   자세히: §값콜백
//   🔑 이것이 **주차 유도**의 자리다. 차가 60 → 40 → 20cm 로 들어오는 동안
//     점유는 계속 "찼다" 라서 ④는 **한 번도 안 불린다.** 거리를 보려면 여기다.
void onSensorValue(ParkingServer& srv, const std::string& spot,
                   const std::string& module, long value) {
    if (module != "B1") return;              // 🔓 값을 내는 센서만 (거르는 것은 네 몫이다)
    (void)spot;

    // 🔑 **값이 흐르는 것을 눈으로 본다.** 점유가 안 바뀌는 동안에도 이 줄이 계속 찍힌다 —
    //   그것이 이 콜백이 ④와 다른 점이다.
    std::cout << "[값] " << module << " " << value << "\n";

    // 🔴 **거리를 표시기에 띄운다 — 다만 *바뀌었을 때만* 보낸다.**
    //   ⚠ 매번 보내면 **슬롯마다 하행 한 건**이다. rid 를 태우고 창을 먹는다.
    //     서버는 그것을 안 막는다(막으면 "그 문턱을 누가 정하나" 가 또 빈 자리가 된다) —
    //   🔑 **거르는 것은 여기, 기여자의 자리다.** 이 두 줄이 그 예다.
    static long shown = -1;
    if (value == shown) return;
    shown = value;
    srv.send("P1", "L2", value);

    // 🔑 매 슬롯 불린다(센서 수만큼). **무거운 일을 여기서 하지 마라** — 서버의 한 박자 안이다.
}
