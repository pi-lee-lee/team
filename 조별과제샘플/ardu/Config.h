...
#define SERVER_IP    "192.168.35.21"  // 🔓 서버를 돌리는 기계의 주소. `ipconfig getifaddr en0` 로 확인한다
                                      // 출처: 루트가 2026-08-20 20:42:04 에 실측. 옛 커밋 f73d8c6 과 동일한 자리다
                                      //   ⚠ 망을 옮기면 서브넷째로 바뀐다. **현장에서 다시 잰다**
// 🔓 **기여자용 기본값은 dev 서버(9991)다. 이 파일에는 이 값 하나만 있다.**
//
// 🔴 **왜 dev 가 기본인가 — 틀렸을 때의 방향이 비대칭이다:**
//   · 9991 인 채로 운영에 쓰면 → **안 붙는다. 즉시 드러난다**
//   · 8888(운영)인 채로 dev 에 쓰면 → 🔴 **운영 서버에 붙는다.** `devid` 가 겹치면
//     **남의 노드를 쫓아내고**, 그런데 **로컬에서는 멀쩡하다** — 합류할 때만 터진다
//   🔑 **틀렸을 때 조용한 쪽을 기본값으로 두지 않는다.**
//
// ⚠ **`SERVER_IP` 와 한 벌이다.** dev 와 운영이 다른 기계면 **둘 다** 바꿔야 한다.
//   "포트만 고치면 된다"로 기억하면 IP 를 안 고치고 헤맨다.
//
// ⚠ `#ifndef` 인 이유: **우리 시험 칩만** 빌드 시점에 덮는다(`-DSERVER_PORT='"8888"'`).
//   그래야 **이 파일이 샘플의 진실 하나만** 들고 있다. 지금 칩이 무엇을 보는지는
//   부팅 `[NET] 대상 …` 줄이 말해 준다 — 소스를 믿지 말고 그 줄을 봐라.
#ifndef SERVER_PORT
