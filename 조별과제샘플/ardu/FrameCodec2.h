#pragma once
// ═════════════════════════════════════════════════════════════════════════
// FrameCodec2.h — 🔴 **`FrameCodec.h` 와 한 몸이다. 성질이 달라서 나뉜 것이 아니다.**
//
// ⚠⚠ **왜 `2` 가 있나 — 이것부터 읽어라. 안 읽으면 합치게 된다:**
//   `MODULE_TABLE` 이 **이 둘 사이에 와야 한다**(앞에서 `ModuleDef` 정의가 오고, 뒤에서 표를 쓰는 코드가 온다).
//   그 표는 그 장치의 자기 구성이라 `client.ino` 에 있는데,
//   **전처리 순서를 바꾸면 `hex` 가 달라진다**(원장 §19 · `espInit` 이 그것으로 물렸다).
//   → 그래서 **같은 자리에서 쪼개고 그 사이에 표를 넣었다.** 순서가 보존되므로 **산출물이 같다.**
//
// 🔴 **합치지 마라 — 합치려면 *표의 위치를 먼저 옮겨라*.**
//   그냥 합치면 표가 뒤로 밀리고 **`hex` 가 달라진다.** 그러면 굽기 축이 하나 는다.
//   🔑 **이 분리의 안전성은 *파일* 이 아니라 *위치* 에 있다.**
//
// 🔮 **언젠가 정리한다면**: `MODULE_TABLE` 을 `client.ino` **머리**로 올리는 것이 목표이고,
//   그때는 `ModuleDef` 정의도 같이 올라가야 하며 **`hex` 가 바뀌므로 굽기 경계가 필요하다.**
//   **그 굽기에 이 파일 통합도 같이 실으면 된다.**
//
// 순서: `FrameCodec.h` → **MODULE_TABLE** → `Modules.h` → **이 파일**
// ═════════════════════════════════════════════════════════════════════════
