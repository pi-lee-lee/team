...
// 🔴🔴 **"못 쟀다" 를 몇 번 참으면 해제하나** — 히스테리시스의 두 번째 구멍을 막는다.
//   `SENSOR_NO_READING`(반사 없음)은 **값이 없어서 히스테리시스가 작동하지 못한다.**
//   그것을 곧바로 "비었다" 로 읽으면 물체가 있는데도 깜빡인다.
//
// 🔑 **실측에서 정했다**(2026-08-20 22:00 굽기 · `V` 프레임 310건 = 150슬롯의 2.07배):
//     B1 이 **72/310 = 23.2%** 를 못 쟀다.  **연속 길이 분포**(슬롯으로 보정):
//       1슬롯 × 14번  ·  2슬롯 × 2번   ← 🔑 **간헐.** 물체는 그대로 있다
//       8슬롯 × 1번   ·  10슬롯 × 1번  ← 🔴 물체가 **실제로 떠난** 구간
//     → **N=3 이 간헐 16/16 을 다 막고 실제 이탈 2/2 는 통과시킨다.** 해제 지연 3.6초
//   ⚠ N=2 는 간헐 14/16 만 막는다. N=4 는 지연만 1.2초 늘고 얻는 것이 없다.
//
// 🔴 **왜 장치에서 막나**(서버가 아니라):
//   ① **"못 쟀다"는 장치만 아는 것이다** — 서버는 빈 칸으로만 본다
//   ② 여기서 막으면 **전선이 안 튄다** → 서버·화면·monitor 가 다 이득
//   🔑 §"한 곳에서만 해라 — 두 곳이면 규칙이 둘 생긴다"
//   ⚠ 그래도 `V` 에는 **빈 칸이 그대로 나간다.** "못 쟀다"는 사실이고 감추지 않는다 —
//     점유 판정만 유지하고 값은 정직하게 비운다.
#ifndef NO_READ_RELEASE_N
#define NO_READ_RELEASE_N 3
#endif
