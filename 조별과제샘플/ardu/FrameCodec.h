...
//   호출부가 하나도 안 남았다. 🔴 남겨 두면 위험하다: 그 함수는 `SENSOR_N`(컴파일 상수)을 돌고
...
//   **비대칭 패턴만이 검증자다.**
//
// ⚠ **`n` 은 `SENSOR_N` 이 아니다.** 자리 유동화 뒤에는 **등록된 모듈 수**다.
//   지금은 값이 같지만 **이름을 갈라 둔다** — 안 그러면 유동화하는 날 조용히 틀린다.
//   폭도 뒤집기 축도 둘 다 `n` 을 쓴다. `SENSOR_N` 을 여기 쓰면 오늘은 맞고 내일 틀린다.
// 🔴 **`n` 의 단일 원천.** 자리 유동화의 축이 여기 하나로 모인다.
//
// 지금은 `SENSOR_N` 을 돌려준다 — **거동 변화 0 이 이 단계의 기대다**(축 3).
// 🔮 유동화하면 **등록된 모듈 수**를 돌려주게 되고, 그때 `S` 폭·hex 폭·뒤집기 축이
//    **전부 이 함수 하나를 따라 움직인다.** 그것이 이 함수를 지금 만드는 이유다.
// ⚠ **`SENSOR_N` 을 인코더에 직접 쓰면 오늘은 맞고 유동화하는 날 조용히 틀린다** —
