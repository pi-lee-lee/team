...
#include "Module.h"      // 🔓 `SENSOR`·`ACTUATOR`·`PIN_NONE` — **표 바로 앞이어야 한다**
...
static constexpr ModuleDef MODULE_TABLE[] PROGMEM = {
  ...
  SENSOR("A1", 2),           // 🔓 2번 핀을 읽는다
  SENSOR("B1", 9),           // 🔓 9번 핀을 읽는다

  // 🔓 **늘리려면 주석을 풀어라. 여기 한 줄이 전부다** — 센서 수·핀은 이 표에서 파생된다
  //   ⚠ 서버 조립 표에도 한 줄을 더해야 한다. 안 하면 `미결속모듈` 로 뜬다
  // SENSOR("A2",  3), SENSOR("B2", 10),
  // SENSOR("A3",  4), SENSOR("B3", 11),
  // SENSOR("A4",  5), SENSOR("B4", 12),
  // SENSOR("A5",  6), SENSOR("B5", A0),
  ...
  ACTUATOR("LD", LED_BUILTIN, cmdLed),   // 🔓 보드 내장 LED. **아무것도 안 사도 된다**
  ACTUATOR("L2", PIN_NONE, cmdL2),       // 🔓 숫자를 받는 표시기 (핀이 없는 모듈도 된다)
