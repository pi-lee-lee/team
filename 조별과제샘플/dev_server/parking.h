...
//       lot.spot("A1").at(0,0).parking().label("1번 자리")     // 주차영역
//             .module("P1","A1").module("P1","B1");             // 모듈 둘
//       lot.spot("E1").label("입구").module("P1","DR");        // 일반영역
...
// 🔴 **`class Gate { enum Kind { IN, OUT }; };` 가 여기 있었다. 2026-08-20 에 없앴다** (v2)
//   입구/출구는 **표시**일 뿐이고 `label("입구")` 이 그것을 한다.
//   자리 종류는 **`parking()` 을 적었나 아닌가** 둘로 줄었다.
//
// 🔑 **덤으로 Windows 충돌이 원인부터 사라졌다** — `<windows.h>` 가 `IN`·`OUT` 을
//   빈 매크로로 정의해 `enum Kind { IN, OUT };` 이 `enum Kind { , };` 로 깨졌다.
//   **`#undef` 는 증상 대처였다.**
// ⚠ **그래도 아래 `#undef` 는 남긴다** — `windows.h` 는 짧은 이름을 많이 정의하고
//   **다음에 다른 이름이 걸릴 수 있다.** 비용은 두 줄이고, 없앴을 때의 손실은 **조용한 빌드 실패**다.
#ifdef IN
#undef IN
#endif
#ifdef OUT
...
class Spot {
public:
    // 🔴 **모듈을 붙인다** (v2 · 2026-08-20). `sensor()`/`actuator()` 를 이것 하나로 합쳤다.
    //
    //   ```
...
public:
    ...
    // 🔴 `gate(id, Gate::Kind)` 가 여기 있었다. **없앴다**(v2) —
    ...
    struct Attach {
        ...
        // 🔴 `bool actuator` 가 여기 있었다. **없앴다**(v2) —
        //   **장치가 `kind` 첫 글자로 말한다**(`I` 관측 · `O` 명령). 선언에서 또 가르지 않는다.
    ...
    struct Area {
        std::string id;
        // 🔴 v2 : **`"parking"` 아니면 `"area"` 둘뿐**이다. `entrance`·`exit` 는 없앴다.
        std::string kind;
