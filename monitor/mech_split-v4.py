...
# 🔴 v4 (2026-08-17 08:4x) — **세 번째 리셋 표지: 부트 ROM 가비지(①)**
#
#   근거(원장 7.53 · A 창 실증): `20:40:34` 사건은 **배너가 유실**돼 `0.0.0.0` 단독으로만
#   잡혔는데, 그 자리에 **부트 ROM 서명이 있다**:
#       20:40:34  b'…`@\xD6\x86@\xF9Rc\xE0\xFCP\x89:\x97…'
#   → **ESP 가 리셋된 것이 맞고 배너만 유실됐다.** ①은 **배너가 유실돼도 남는 표지**다.
#
#   ⚠ **①은 정상 부팅에도 뜬다**(우리 DTR 포함) — **단독 판정 절대 금지.**
#     배너가 죽은 이유(1차 판별자)와 같은 함정이다. **모집단을 사건으로 좁힌 뒤의 OR 항**이다(3.7-b).
#
#   ⚠ 앵커는 **`@\xF9`** 다. `86 40 F9` 로 잡으면 놓친다 — 앞 바이트가 `\x86`·`\x84` 로 흔들린다
#     (표본화가 매번 다르다). 루트가 예시 한 줄에서 상수를 뽑았다가 정정한 자리.
#
#   ⚠ **가비지는 둘이다**(원장 7.52). 앵커 **있는** 것만 ①(리셋)이고,
#     앵커 **없는** 것은 ②(UART 손상)이라 **리셋 표지가 아니다.** ②를 쓰면 최대 6초 앞으로 당겨진다.
MARK_BOOTROM = b"@\\xF9"        # 로그가 이스케이프 텍스트라 4문자다
MARK_NOIP = b"0.0.0.0"
...
def main() -> int:
    ...
    print("# 판별자 v4: 사건 창 안에서 (`0.0.0.0` OR 배너 OR **부트ROM `@\\xF9`**) — 셋 다 단독 판정 금지")
    ...
    for t, _ in events:
        ...
        has_bootrom = any(MARK_BOOTROM in r for r in near)  # ① 부트 ROM 서명 (v4 추가)
        ...
        reset_like = has_banner or has_noip_raw or has_bootrom
