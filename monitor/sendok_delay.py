...
⚠ 갈래를 반드시 갈라라
  ...
  다만 비정상·짝짓기실패 건수도 낸다 — **분모가 없으면 꼬리를 말할 수 없다.**

🔴 2026-08-16 정정 — 이 도구가 **병리 구간을 통째로 놓치고 있었다**
  자해 2건(19:25:43 · 20:54:02)에서 **문제의 `CIPSEND` 이후 6초 동안 어떤 `SEND OK` 도 없었다.**
  그런데 그 6초는 **정상(최대 Δ 2초)에도 비정상(최대 Δ 3초)에도 안 들어간다** —
  재시도 `CIPSEND` 가 먼저 와서 짝짓기가 깨져 `짝짓기 실패` 칸으로 빠졌기 때문이다.
  → **즉 "정상 분포의 꼬리"만 보고 임계값을 정하면 병리 구간이 표본에서 빠진 채로 정해진다.**
  ⚠ 그 6초가 "문제의 CIPSEND 의 응답이 늦은 것"인지는 **말할 수 없다.**
     로그는 `SEND OK` 가 어느 명령의 응답인지 표시하지 않는다. 가장 최근 명령에 붙이면 3초다.
     받칠 수 있는 표현은 **"그 6초 동안 어떤 `SEND OK` 도 없었다"** 까지다.
...
def main() -> int:
    ...
    print(f"\n## 표본  정상 {n_norm:,} · 비정상(busy/Error/RESYNC 낌) {n_bad:,}"
          f" · **짝짓기 실패 {unpaired:,}**")
    print("   ⚠ '짝짓기 실패' = **다음 `CIPSEND` 가 먼저 와서 짝을 못 지은 것**이다.")
    print("      ~~'SEND OK 미도달'~~ 이라고 부르지 마라 — 그건 '응답이 영영 안 왔다'로 읽히는데")
    print("      **이 도구는 늦게라도 왔는지를 못 가른다**(로그가 응답의 주인을 표시하지 않는다).")
    print("      🔴 이 칸을 `okto` 기대치의 분모로 쓸 때 특히 조심하라(2026-08-16 정정).")
    if n_norm < 100:
