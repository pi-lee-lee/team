...
# A 창 정의와 기준 발생률 — 주기 보고에서 **예상 대비 실제**를 내기 위해.
# 3.00/h 는 2026-08-16 17:53~20:13 (7건/2.33h) 실측이다. **추정이 아니라 측정값**이지만
# ⚠ 표본 7건짜리라 오차가 크다. 예상에서 벗어났다고 곧장 "변했다"로 읽지 마라 —
#    그 판정은 `rate_stability.py` 가 검정력과 함께 낸다(원장 1.15).
A_START = datetime(2026, 8, 16, 17, 53, 7)
A_END = datetime(2026, 8, 17, 1, 53, 0)
BASE_RATE_H = 3.00
...
def main() -> int:
    ...
    while True:
        ...
        if (now - started).total_seconds() >= REPORT_EVERY_MIN * 60:
            ...
            # 예상 대비 실제 — 크게 어긋나면 그 자체가 발생률 변화의 신호다(루트 지시)
            el_h = max(0.0, (now - A_START).total_seconds() / 3600)
            exp_now = BASE_RATE_H * el_h
            full_h = (A_END - A_START).total_seconds() / 3600
            proj = (ev / el_h * full_h) if el_h > 0 else 0
            print(f"   A 경과 {el_h:.2f}h · 사건 {ev} (예상 {exp_now:.1f}, 기준 {BASE_RATE_H}/h)")
            print(f"   → 이 속도면 A 종료 시 **약 {proj:.0f}건** (검정력 표 기준선은 24건)")
            print("   ⚠ 예상에서 벗어났다고 곧장 '변했다'로 읽지 마라 — 판정은 rate_stability.py 가")
            print("      검정력과 함께 낸다. 여기 숫자는 **보고용 눈금**이다(원장 1.15).")
