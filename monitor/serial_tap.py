...
def main() -> int:
    ...
    # ⚠ **생존 신호는 "언제 죽었나"만 답한다. "왜 죽었나"는 못 답한다**(루트 지적 · 2026-08-18).
    #   그 절반은 **띄우는 방법**으로 메운다 — 파이썬을 sh -c 로 감싸 종료 코드를
    #   에러 파일에 남기면 된다 (실행 뒤 exit 상태를 echo 한다).
    #
    #   exit=0   정상 종료          exit=1   파이썬 예외(그 위에 역추적이 찍힌다)
    #   exit=143 SIGTERM(=우리 pkill)   exit=137 SIGKILL(OOM·강제)   그 밖 = 그 밖
    #   🔴 2026-08-18 창 J 는 에러 파일이 **완전히 비어 있어** 이 셋을 하나도 못 갈랐다.
