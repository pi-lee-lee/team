...
다만 플래싱은 **관측 경계를 만드는 개입**이다. 순서를 지켜라:

1. **`arduino/INTERVENTIONS.md` 에 먼저 적는다** — 시각(초 단위)·무엇을·왜. 굽기 전에.
2. **monitor 에게 통보한다.** 시리얼 포트를 쓰므로 tap 과 충돌하고, DTR 리셋이 관측 창을 자른다.
   포트를 잡거나 놓는 일은 **배타적 자원**이다(CLAUDE.md 의 그 절을 따른다).
3. **굽고 나서 대조한다** — 방금 구운 hex 와 저장소 판본을 바이트로 비교하고 커밋 해시를 적는다.
   "무엇이 칩에 있는가"를 모르면 이후 관측을 어느 소스에도 귀속할 수 없다.
4. **소스를 고쳤으면 칩과 갈라졌다는 것을 명시한다.** 옛 "IDENTICAL" 기록을 현재로 인용하지 마라.
